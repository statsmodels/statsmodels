...
class TestInteractionPlot:
    ...
    def test_formatting_errors(self, close_figures):
        with pytest.raises(ValueError):
            interaction_plot(self.weight, self.duration, self.days, markers=["D"])
        with pytest.raises(ValueError):
            interaction_plot(
                self.weight, self.duration, self.days, colors=["b", "r", "g"]
            )
        with pytest.raises(ValueError):
            interaction_plot(
    ...
    def test_plottype(self, close_figures):
        ...
        assert_equal(isinstance(fig, plt.Figure), True)
        with pytest.raises(ValueError):
            interaction_plot(self.weight, self.duration, self.days, plottype="unknown")
