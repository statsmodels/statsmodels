...
def test_outliers_gy():
    ...
    res1 = [0.01786544429608583, 8.416367423905008, 17.0, 42.0, 5.023886187314888]
    ...
    res1 = [0.008598069552744558, 12.605802816238732, 2.0, 9.0, 9.348403604496148]
    assert_allclose(res, res1, rtol=1e-13)
...
class TestOGKMad:
    ...
    def test(self):
        ...
        assert_allclose(res1.cov_raw, res2.cov_raw, rtol=1e-08)
        assert_allclose(res1.loc_raw, res2.center_raw, rtol=1e-08)
...
class TestOGKTau(TestOGKMad):
    ...
    def test(self):
        ...
        assert_allclose(res1.cov_raw, res2.cov_raw * fact, rtol=1e-08)
...
def test_tyler():
    ...
    center = np.array(
        [1.5583333333333333, 1.8033333333333335, 1.66, -0.0866666666666667]
    )
    k_vars = len(center)
...
def test_cov_ms():
    ...
    res = mod.fit()
    assert_allclose(res.mean, mean_r, rtol=1e-05)
    assert_allclose(res.shape, shape_r, rtol=1e-05)
    assert_allclose(res.cov, cov_r, rtol=1e-05)
    assert_allclose(res.scale, scale_r, rtol=1e-05)
...
def test_covdetmcd():
    ...
    mean_dmcd_r = np.array([1.7725, 2.205, 1.5375, -0.0575])
    mod = robcov.CovDetMCD(dta_hbk)
    res = mod.fit(40, maxiter_step=100, reweight=False)
    assert_allclose(res.mean, mean_dmcd_r, rtol=1e-05)
    assert_allclose(res.cov, cov_dmcd_r, rtol=1e-05)
    ...
    assert_allclose(res.mean, mean_dmcdw_r, rtol=1e-05)
    shape = res.cov / np.trace(res.cov)
    shape_r = cov_dmcdw_r / np.trace(cov_dmcdw_r)
    assert_allclose(shape, shape_r, rtol=1e-05)
...
def test_covdetmm():
    ...
    mean_dmm_r = np.array(
        [1.538864342046, 1.8027582110408, 1.6811517253521, -0.0755069488908]
    )
    c = 5.81031555752526
    mod = robcov.CovDetMM(dta_hbk, norm=robnorms.TukeyBiweight(c=c))
    res = mod.fit()
    assert_allclose(res.mean, mean_dmm_r, rtol=1e-05)
    assert_allclose(res.cov, cov_dmm_r, rtol=1e-05, atol=1e-05)
    mod = robcov.CovDetMM(dta_hbk)
    res = mod.fit()
    assert_allclose(res.mean, mean_dmm_r, rtol=0.001)
    assert_allclose(res.cov, cov_dmm_r, rtol=0.001, atol=0.001)
...
def test_robcov_SMOKE():
    ...
    r = robcov._cov_iter(x, robcov.weights_quantile, weights_args=(0.5,), rescale="med")
    assert_allclose(r.cov, cov, rtol=0.5)
    ...
    for _, res in enumerate(res_li):
        if hasattr(res, "n_iter") or hasattr(res, "cov_ogk_raw"):
            ...
            assert_allclose(c1, cov_clean, rtol=1e-08)
