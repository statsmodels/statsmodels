    ...
    def __init__(self, R, q=None):
        ...
        if q is not None:
            ...
            try:
                ...
                raise ValueError(
                    "possibly inconsistent constraints. error "
                    "generated by\n%r" % (e,)
                ) from e
        else:
            self.constant = 0
...
def fit_constrained(
    model, constraint_matrix, constraint_values, start_params=None, fit_kwds=None
):
...
def fit_constrained_wrap(model, constraints, start_params=None, **fit_kwds):
    ...
    res = self.fit(
        start_params=params, maxiter=0, warn_convergence=False
