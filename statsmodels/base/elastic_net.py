...
def _l1_slsqp(
    ...
    def f(params, *args):
        return -model.loglike(params, *args) / nobs

    def score(params, *args):
        return -model.score(params, *args) / nobs

    alpha = np.array(alpha)
    assert alpha.min() >= 0

    kwargs = dict(alpha=alpha)
    # no need to rescale because l1_slsqp solves the scaled problem
    kwargs["alpha_rescaled"] = kwargs["alpha"]
...
def fit_elasticnet(
    ...
    **kwargs,
    ...
    method : {'elastic_net', 'l1_slsqp'}
        The method to use for fitting the model. 'l1_slsqp' only supports
        L1_wt = 1.0.
    maxiter : int
    ...
    k_exog = model.exog.shape[1]
    if method == "l1_slsqp":
        assert L1_wt == 1.0, "L1_wt must be 1.0 for l1_slsqp"

        trim_mode = kwargs.get("trim_mode", "auto")
        auto_trim_tol = kwargs.get("auto_trim_tol", 0.01)
        size_trim_tol = kwargs.get("size_trim_tol", 1e-4)
        qc_tol = kwargs.get("qc_tol", 0.03)
        qc_verbose = kwargs.get("qc_verbose", False)
        ...
        cnvrg_tol = kwargs.get("cnvrg_tol", 1e-7)
        zero_tol = kwargs.get("zero_tol", 1e-8)
        check_step = kwargs.get("check_step", True)
