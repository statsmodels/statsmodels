...
def func(x, a):
    f = (x - a) ** 3
    if DEBUG:
        print(f"evaluating at {x:g}, fval = {f:f}")
    return f
...
def func_nan(x, a, b):
    ...
    f[x < b] = np.nan
    if DEBUG:
        print(f"evaluating at {x:f}, fval = {f:f}")
    return f
...
def funcn(x, a):
    f = -((x - a) ** 3)
    if DEBUG:
        print(f"evaluating at {x:g}, fval = {f:g}")
    return f
...
if __name__ == "__main__":
    ...
    if run_all:
        ...
        print(
            brentq_expanding(
                func, args=(1.234e30,), xtol=1e10, increasing=True, maxiter_bq=200
            )
        )
