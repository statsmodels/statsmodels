...
if __name__ == "__main__":
    ...
    print(
        "scale - dgp realised, true",
        (y - (f.y_true + x2.sum(1))).std(),
        2 * f.het_scale(1),
    )
    ...
    print(
        np.cov(np.column_stack((xb_est, mean, resid)), rowvar=False)
    ...
    ax.plot(
        f.x[sortidx], f.y[sortidx], "o", color="b", lw=2, alpha=alpha, label="observed"
    )
    ax.plot(
        f.x[sortidx],
        f.y_true[sortidx],
    ...
    ax = fig.add_subplot(1, 1, 1)
    ax.plot(
        f.x[sortidx], y[sortidx], "o", color="b", lw=2, alpha=alpha, label="observed"
    )
    ax.plot(
        f.x[sortidx],
        f.y_true[sortidx],
