...
@cython.wraparound(False)
@cython.cdivision(True)
def fast_linbin_weights(np.ndarray[DOUBLE] X, np.ndarray[DOUBLE] W, double a, double b, int M, int trunc=1):
    """
    Linear Binning as described in Fan and Marron (1994) with added weighting
    """
    cdef:
        Py_ssize_t i, li_i
        int nobs = X.shape[0]
        ...
        np.ndarray[INT] li = lxi.astype(int)
        np.ndarray[DOUBLE] rem = lxi - li
        np.ndarray[DOUBLE] weights = W


    for i in range(nobs):
        li_i = li[i]
        if li_i > 1 and li_i < M:
            gcnts[li_i] = gcnts[li_i] + (1 - rem[i])*W[i]
            gcnts[li_i+1] = gcnts[li_i+1] + rem[i]*W[i]
        if li_i > M and trunc == 0:
            gcnts[M] = gcnts[M] + 1*W[i]
    return gcnts
