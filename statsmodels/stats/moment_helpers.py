...
def __convert_to_multidim(x):
    if any([isinstance(x, list),isinstance(x, tuple)]):
        return np.array(x)
    elif isinstance(x, np.ndarray):
        return x
    else:
        return x #something strange was passed and the function probably will fall, maybe insert an exception?


def __convert_from_multidim(x, totype=list):
    if len(x.shape) < 2:
        return totype(x)
    return x.T
...
def mc2mnc(mc):
    ...
    return __convert_from_multidim(res) #for backward compatibility convert 1-dim output to list/tuple
...
def mvsk2mc(args):
    ...
    def _local_counts(args):
        ...
        return tuple(cnt)
    res = np.apply_along_axis(_local_counts, 0, X)
    return __convert_from_multidim(res, tuple) #for backward compatibility convert 1-dim output to list/tuple
