...
def test_oaxaca_results():
    ...
    endow, coef, inter, gap = model.three_fold().params
    unexp, exp, gap = model.two_fold().params
    endow_var, coef_var, inter_var = model.three_fold(True).std
