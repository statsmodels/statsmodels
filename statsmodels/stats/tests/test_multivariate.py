...
def test_mv_mean():
    ...
    nobs = len(x[:, 1:])
    ci1 = confint_mvmean_fromstats(
        dw.mean, np.diag(dw.var), nobs, lin_transf=np.eye(4), alpha=0.05
    )
    ci2 = confint_mvmean_fromstats(
        dw.mean, dw.cov, nobs, lin_transf=np.eye(4), alpha=0.05
    )
...
def test_confint_simult():
    ...
    res_constr = confint_mvmean_fromstats(
        m, cov, nobs, lin_transf=[0, 1, -1], simult=True
    )
