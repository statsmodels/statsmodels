...
class TestDeltacovOLS(object):
    ...
    def test_method(self):
        ...
        assert_allclose(nlm.se_vectorized(), nlm.se_vectorized(), rtol=1e-14)
        assert_allclose(nlm.predicted(), nlm.predicted(), rtol=1e-14)
        df = res.df_resid
    ...
    def test_ttest(self):
        ...
        se = nl.se_vectorized()
        assert_allclose(predicted, fun(res.params), rtol=1e-14)
        assert_allclose(se, np.sqrt(np.diag(nl.cov())), rtol = 1e-14)

        tt = res.t_test(x, use_t=False)
        assert_allclose(predicted, tt.effect, rtol=1e-14)
        assert_allclose(se, tt.sd, rtol=1e-14)
        assert_allclose(nl.conf_int(), tt.conf_int(), rtol=1e-14)
        t1 = nl.summary()
        t2 = tt.summary()
        ...
        assert_allclose(nl.conf_int(use_t=True, df=df), tt.conf_int(), rtol=1e-14)
        t1 = nl.summary(use_t=True, df=df)
        t2 = tt.summary()
        # equal because nl.summary uses also ContrastResults
        assert_equal(str(t1), str(t2))
    ...
    def test_diff(self):
        ...
        assert_allclose(nl.predicted(), res.params[0], rtol=1e-14)
        assert_allclose(nl.se_vectorized(), res.bse[0], rtol=1e-14)
...
def test_deltacov_margeff():
    ...
    # 2d f doesn't work correctly, se_vectorized and predicted are 2d column vector
    f = lambda p: np.squeeze(res_poi.model._derivative_exog(p, res_poi.model.exog.mean(0)[None, :]))
    ...
    #print(nlp.summary())
    # margeff excludes constant, last parameter in this case
    assert_allclose(nlp.se_vectorized()[:-1], marg.margeff_se, rtol=1e-14)
    assert_allclose(nlp.predicted()[:-1], marg.margeff, rtol=1e-14)

    nlpm = res_poi._wald_test_nl(f)
    # margeff excludes constant, last parameter in this case
    assert_allclose(nlpm.se_vectorized()[:-1], marg.margeff_se, rtol=1e-14)
    assert_allclose(nlpm.predicted()[:-1], marg.margeff, rtol=1e-14)
