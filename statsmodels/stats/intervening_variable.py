...
sychol Methods. 2002 Mar;7(1):83-104. doi: 10.1037/1082-989x.7.1.83.
...
class InterveningVariable:
    ...
    outcome_model : statsmodels model
        ...
        the treatment variable, the mediation variable, and any other covariates 
        of interest.
    mediator_model : statsmodels model
        Regression model for the mediator variable. Exogenous variables must include 
        the treatement variable. For a rigorous analysis, include the same 
        covariates as in outcome_model
    treatment_name : str
    ...
    def __init__(self, outcome_model, mediator_model, treatment_name,
                 outcome_fit_kwargs=None, mediator_fit_kwargs=None):

        self.mediator_model = mediator_model
        self.outcome_model = outcome_model

        if not treatment_name in mediator_model.exog_names:
            raise ValueError(f'Treatment variable {treatment_name} not in '
                             'exogenous variables of mediator_model.')
        else:
            self.tx_indx_in_med_model = \
                self.mediator_model.exog_names.index(treatment_name)
        ...
        self.med_indx_in_outcome_model = \
            self.outcome_model.exog_names.index(self.mediator_name)
    ...
    def _get_indirect(self, outcome_model_fitted, mediator_model_fitted):
        treatment_mediator_coeff = \
            mediator_model_fitted.params[self.tx_indx_in_med_model]
        mediator_indep_coeff = \
            outcome_model_fitted.params[self.med_indx_in_outcome_model]
        treatment_mediator_stderr = \
            mediator_model_fitted.bse[self.tx_indx_in_med_model]
        mediator_indep_stderr = \
            outcome_model_fitted.bse[self.med_indx_in_outcome_model]

        indirect_effect = \
            treatment_mediator_coeff * mediator_indep_coeff
        indirect_stderr = \
            np.sqrt((treatment_mediator_coeff**2)*(mediator_indep_stderr**2)
    ...
    def fit(self, n_reps=1000):
        ...
        self.direct_effect = \
            outcome_model_fitted.params[self.tx_indx_in_outcome_model]
        self.direct_stderr = \
            outcome_model_fitted.bse[self.tx_indx_in_outcome_model]

        mediator_model_fitted = self._fit_model(
            self.mediator_model, self._mediator_fit_kwargs)
        self.sobel_indirect_effect, self.sobel_indirect_stderr = \
        ...
        for _ in range(n_reps):
            ...
            sample_outcome_model_fitted = self._fit_model(self.outcome_model,
                                                          self._outcome_fit_kwargs,
                                                          boot=True, sample=sample)
            sample_mediator_model_fitted = self._fit_model(self.mediator_model,
                                                           self._mediator_fit_kwargs,
                                                           sample=sample)
            sample_direct_effect = \
                sample_outcome_model_fitted.params[self.tx_indx_in_outcome_model]
            sample_indirect_effect, sample_indirect_stderr = \
                self._get_indirect(sample_outcome_model_fitted,
                                   sample_mediator_model_fitted)
            boot_sample_indirect_effects.append(sample_indirect_effect)
            boot_sample_indirect_proportions.append(
                sample_indirect_effect/(sample_direct_effect + sample_indirect_effect))
        ...
        self.boot_indirect_stderr = \
            np.std(self.boot_sample_indirect_effects)

        self.boot_indirect_proportion = self.boot_sample_indirect_proportions.mean()
        self.boot_indirect_proportion_stderr = \
            np.std(self.boot_sample_indirect_proportions)

        return InterveningVariableResults(self.direct_effect,
                                          self.direct_stderr,
                                          self.sobel_indirect_effect,
                                          self.sobel_indirect_stderr,
                                          self.boot_indirect_stderr,
                                          self.boot_sample_indirect_effects,
                                          self.boot_indirect_proportion,
                                          self.boot_indirect_proportion_stderr,
...
class InterveningVariableResults:
    ...
    def __init__(self, direct_effect, direct_stderr,
                 sobel_indirect_effect, sobel_indirect_stderr,
                 boot_indirect_stderr,
                 boot_sample_indirect_effects,
                 boot_indirect_proportion, boot_indirect_proportion_stderr,
                 boot_sample_indirect_proportions):
        ...
        self.boot_indirect_proportion = boot_indirect_proportion
        self.boot_indirect_proportion_stderr = boot_indirect_proportion_stderr
        self.boot_sample_indirect_proportions = boot_sample_indirect_proportions
    ...
    def summary(self, alpha=0.05):
        ...
        ----------
        alpha : int, optional
           1 - (confidence level). I.e. alpha = 0.05 will compute 95% confidence 
           intervals. The default is 0.05.

        Returns
        -------
        DataFrame
            A dataframe containing the results of the intervening variable analysis.

        """
        z_score = st.norm.ppf(1 - alpha/2)

        self.direct_effect_lb = \
            self.direct_effect - (z_score * self.direct_stderr)
        self.direct_effect_ub = \
            self.direct_effect + (z_score * self.direct_stderr)
        direct_info = \
            [self.direct_effect, self.direct_stderr,
             self.direct_effect_lb, self.direct_effect_ub]

        self.sobel_indirect_effect_lb = \
            self.sobel_indirect_effect - (z_score * self.sobel_indirect_stderr)
        self.sobel_indirect_effect_ub = \
            self.sobel_indirect_effect + (z_score * self.sobel_indirect_stderr)
        sobel_indirect_info = \
            [self.sobel_indirect_effect, self.sobel_indirect_stderr,
             self.sobel_indirect_effect_lb, self.sobel_indirect_effect_ub]

        self.boot_indirect_effect_lb = \
            np.percentile(self.boot_sample_indirect_effects, 100 * alpha / 2)
        self.boot_indirect_effect_ub = \
            np.percentile(self.boot_sample_indirect_effects,
                          100 * (1 - alpha / 2))
        boot_indirect_info = \
            [self.sobel_indirect_effect, self.boot_indirect_stderr,
             self.boot_indirect_effect_lb, self.boot_indirect_effect_ub]

        self.boot_indirect_proportion_lb = \
            np.percentile(self.boot_sample_indirect_proportions, 100*alpha/2)
        self.boot_indirect_proportion_ub = \
            np.percentile(self.boot_sample_indirect_proportions,
                          100 * (1 - alpha / 2))
        boot_indirect_proportion_info = \
            [self.boot_indirect_proportion, self.boot_indirect_proportion_stderr,
             self.boot_indirect_proportion_lb, self.boot_indirect_proportion_ub]

        columns = ['value', 'std err', 'lower ci bound', 'upper ci bound']
        index = ['direct effect', 'sobel indirect effect',
                 'bootstrap indirect effect', 'bootstrap indirect/total']

        return pd.DataFrame([direct_info, sobel_indirect_info,
                             boot_indirect_info, boot_indirect_proportion_info],
