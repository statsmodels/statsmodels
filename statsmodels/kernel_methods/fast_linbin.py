...
def _fast_bin(fct, X, bounds, M, weights, bin_type, out):
    ...
    try:
        ...
        if weights.ndim != 1 or weights.shape[0] != X.shape[0]:
            raise ValueError("Weights must be a single value or an array "
    ...
    if out is None:
        ...
        raise ValueError(
            "Error, out array must be a 1D float array with length M")
    try:
        a, b = bounds
        a = float(a)
        b = float(b)
...
if _fast_bin.__doc__:
    fast_linbin.__doc__ = """
        ...
        Unless the bin is discrete, for a point :math:`x` between bins
        ...
        - For continuous bins, the bins are placed at :math:`\{a+\delta/2,
          \ldots, a+k \delta + \delta/1, \ldots b-\delta/2\}` with :math:`delta
          = \frac{M-1}{b-a}`.
        ...
        \delta, \ldots, b-\delta\}` with :math:`\delta = \frac{M}{b-a}` and
        there is a virtual bin in :math:`b` which is fused with :math:`a`.
        """
...
def _fast_bin_nd(fct, X, bounds, M, weights, bin_type, out):
    ...
    if D > max_d:
        raise ValueError(
            "Error, you cannot have more than {0} dimensions, and you have {1}"
            .format(max_d, D))
    ...
    if out is None:
        ...
        raise ValueError(
            "Output array is either of the wrong size or the wrong type")
...
def fast_linbin_nd(X, bounds, M, weights=1., bin_type='B', out=None):
    ...
    if X.shape[1] == 3:
        return _fast_bin_nd(_cy_fast_linbin.fast_linbin_3d, X, bounds, M,
                            weights, bin_type, out)
    return _fast_bin_nd(_cy_fast_linbin.fast_linbin_nd, X, bounds, M, weights,
                        bin_type, out)
...
if _fast_bin_nd.__doc__:
    fast_linbin_nd.__doc__ = r"""
        ...
        - For reflected or un-bounded bins, the bins are placed at
          :math:`\{a+\delta/2, \ldots, a+k \delta + \delta/1, \ldots
