...
def iris():
    ...
    return np.genfromtxt(
        Path(cur_dir).joinpath("results", "iris.csv"), delimiter=",", skip_header=1
    )
...
class CheckModelResultsMixin:
    ...
    def test_standard_errors(self):
        assert_allclose(
            self.res1.bse, self.res2.bse, atol=10 ** (-self.decimal_bse), rtol=1e-05
        )
    ...
    def test_residuals(self):
        ...
        assert_allclose(resids, resid2, rtol=1e-06, atol=atol)
    ...
    def test_pearson_chi2(self):
        if hasattr(self.res2, "pearson_chi2"):
            assert_allclose(
                self.res1.pearson_chi2, self.res2.pearson_chi2, atol=1e-06, rtol=1e-06
            )

    def test_prsquared(self):
        if hasattr(self.res2, "prsquared"):
            assert_allclose(
                self.res1.pseudo_rsquared(kind="mcf"), self.res2.prsquared, rtol=0.05
            )
...
class CheckComparisonMixin:
    ...
    def test_get_prediction(self):
        ...
        assert_allclose(
            predd.summary_frame().values, pred1.summary_frame().values, rtol=1e-06
        )
...
class TestGaussianLog(CheckModelResultsMixin):
    ...
    def setup_class(cls):
        ...
        GaussLog_Model = GLM(
            cls.lny, cls.X, family=sm.families.Gaussian(sm.families.links.Log())
        )
...
class TestGlmBernoulli(CheckModelResultsMixin, CheckComparisonMixin):
    ...
    def test_score_r(self):
        ...
        select.pop(7)
        res1b = GLM(
            res2.endog, res2.exog.iloc[:, select], family=sm.families.Binomial()
        ).fit()
...
class TestGlmPoissonOffset(CheckModelResultsMixin):
    ...
    def setup_class(cls):
        ...
        cls.res1 = GLM(
            data.endog, data.exog, family=sm.families.Poisson(), exposure=exposure
        ).fit()
...
def test_formula_missing_exposure():
    ...
    with pytest.raises(ValueError):
        GLM(df.Foo, df[["constant", "Bar"]], exposure=exposure, family=family)
...
def test_gradient_irls(family_and_link, binom_version):
    ...
    for max_start_irls, start_params in ((0, rslt_irls.params), (3, None)):
        ...
        assert_allclose(rslt_gradient.bse, rslt_irls.bse, rtol=0.2, atol=5e-05)
        ...
        assert_allclose(rslt_gradient_eim.bse, rslt_irls.bse, rtol=5e-05, atol=0)
...
class CheckWtdDuplicationMixin:
    ...
    def test_null_deviance(self):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DomainWarning)
            assert_allclose(
                self.res1.null_deviance, self.res2.null_deviance, atol=1e-06, rtol=1e-06
            )
    ...
    def test_tpvalues(self):
        ...
        assert_allclose(
            self.res1.conf_int(), self.res2.conf_int(), atol=1e-06, rtol=1e-06
        )
...
class TestWtdGlmPoissonClu(CheckWtdDuplicationMixin):
    ...
    def setup_class(cls):
        ...
        fit_kwds = dict(
            cov_type="cluster", cov_kwds={"groups": gid, "use_correction": False}
        )
        import warnings
        ...
        with warnings.catch_warnings():
            ...
            fit_kwds = dict(
                cov_type="cluster", cov_kwds={"groups": gidr, "use_correction": False}
            )
            cls.res2 = GLM(
...
class TestWtdGlmGammaScale_X2(CheckWtdDuplicationMixin):
    ...
    def setup_class(cls):
        ...
        cls.res2 = GLM(cls.endog_big, cls.exog_big, family=family_link).fit(scale="X2")
...
class CheckTweedie:
    ...
    def test_resid(self):
        ...
        assert_allclose(
            np.concatenate(
                (self.res1.resid_response[:17], [self.res1.resid_response[idx1]])
            ),
            np.concatenate(
                (self.res2.resid_response[:17], [self.res2.resid_response[idx2]])
            ),
            rtol=1e-05,
            atol=1e-05,
        )
        ...
        assert_allclose(
            np.concatenate(
                (self.res1.resid_deviance[:17], [self.res1.resid_deviance[idx1]])
            ),
            np.concatenate(
                (self.res2.resid_deviance[:17], [self.res2.resid_deviance[idx2]])
            ),
            rtol=1e-05,
            atol=1e-05,
        )
...
class CheckTweedieSpecial:
    ...
    def test_resid(self):
        assert_allclose(
            self.res1.resid_response, self.res2.resid_response, rtol=1e-05, atol=1e-05
        )
        assert_allclose(
            self.res1.resid_pearson, self.res2.resid_pearson, rtol=1e-05, atol=1e-05
        )
        assert_allclose(
            self.res1.resid_deviance, self.res2.resid_deviance, rtol=1e-05, atol=1e-05
        )
        assert_allclose(
            self.res1.resid_working, self.res2.resid_working, rtol=1e-05, atol=1e-05
        )
        assert_allclose(
...
class TestRegularized:
    ...
    def test_regularized(self):
        ...
        for dtype in ("binomial", "poisson"):
            cur_dir = Path(__file__).resolve().parent
            data = np.loadtxt(
                Path(cur_dir).joinpath("results", "enet_%s.csv" % dtype), delimiter=","
            )
            endog = data[:, 0]
            exog = data[:, 1:]
            fam = {"binomial": sm.families.Binomial, "poisson": sm.families.Poisson}[
                dtype
            ]
            for j in range(9):
...
class TestConvergence:
    ...
    def _when_converged(
        self, atol=1e-08, rtol=0, tol_criterion="deviance", *, result=None
    ):
...
def test_glm_summary2_method():
    ...
    assert re.compile("Method:\\s+bfgs").findall(str(summ))
