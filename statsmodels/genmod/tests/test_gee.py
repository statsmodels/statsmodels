...
class TestGEE(object):
    ...
    def test_offset_formula(self):
        ...
        D = pd.DataFrame({"Y": Y, "X1": X1, "X2": X2, "groups": groups,
                          "offset": offset, "exposure": exposure})

        fml = "Y ~ X1 + X2"
        model1 = GEE.from_formula(fml, groups, D, family=Poisson(),
                                  offset="offset")
        result1 = model1.fit()
        assert_equal(result1.converged, True)

        model2 = GEE.from_formula(fml, groups, D, family=Poisson(),
                                  offset=offset)
        result2 = model2.fit(start_params=result1.params)
        assert_allclose(result1.params, result2.params)
        assert_equal(result2.converged, True)

        model3 = GEE.from_formula(fml, groups, D, family=Poisson(),
                                  exposure=exposure)
        ...
        model4 = GEE.from_formula(fml, groups, D, family=Poisson(),
                                  exposure="exposure")
        ...
        model5 = GEE.from_formula(fml, groups, D, family=Poisson(),
                                  exposure="exposure", offset="offset")
        result5 = model5.fit()
        assert_equal(result5.converged, True)

        model6 = GEE.from_formula(fml, groups, D, family=Poisson(),
                                  offset=2*offset)
