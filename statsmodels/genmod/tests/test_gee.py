...
class TestGEE:
    ...
    def test_nested_linear(self):
        ...
        assert_allclose(
            smry.Variance, np.r_[1.043878, 0.611656, 1.421205], atol=1e-05, rtol=1e-05
        )
    ...
    def test_nested_pandas(self):
        ...
        assert_allclose(
            smry.Variance,
            np.r_[1.437299, 4.421543, 8.905295, 0.25848],
            atol=1e-05,
            rtol=1e-05,
        )
    ...
    def test_formula_environment(self):
        ...
        result_formula = gee.GEE.from_formula(
            "Y ~ times_two(X1)", groups, data, time=Time, family=family, cov_struct=va
        ).fit()
    ...
    def test_stationary_grid(self):
        ...
        assert_allclose(
            cs.covariance_matrix(np.r_[1, 1, 1], 0)[0].sum(), 6.463353828514945
        )
        assert_allclose(
            result.params, np.r_[4.463968, -0.0386674], rtol=1e-05, atol=1e-05
        )
