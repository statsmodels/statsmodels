...
class UnobservedComponents(MLEModel):
    ...
    **Regression effects**
    ...
    def _res_classes(self):
        return {
            "fit": (UnobservedComponentsResults, UnobservedComponentsResultsWrapper)
        }
    ...
    def transform_params(self, unconstrained):
        ...
        constrained[offset : offset + self.k_exog] = unconstrained[
    ...
    def untransform_params(self, constrained):
        ...
        # Nothing to do with betas
        unconstrained[offset : offset + self.k_exog] = constrained[
            offset : offset + self.k_exog
        ]
...
class UnobservedComponentsResults(MLEResults):
    ...
    def freq_seasonal(self):
        ...
        if spec.freq_seasonal:
            ...
            for ix, h in enumerate(spec.freq_seasonal_harmonics):
                ...
                filtered_cov = np.sum(
                    [
                        self.filtered_state_cov[offset + j, offset + j]
                        for j in states_in_sum
                    ],
                    axis=0,
                )
    ...
    def summary(self, alpha=0.05, start=None):
        ...
        return super().summary(
            alpha=alpha,
            start=start,
            title="Unobserved Components Results",
