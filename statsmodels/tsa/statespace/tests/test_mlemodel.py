...
from statsmodels.tsa.statespace import kalman_filter, kalman_smoother, sarimax, varmax
...
def test_score_analytic_ar1():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        assert_allclose(
            mod._hessian_complex_step(params) * 2, analytic_hessian, atol=0.1
        )
        assert_allclose(
            mod._hessian_finite_difference(params) * 2, analytic_hessian, atol=0.1
        )
...
def test_diagnostics_nile_eviews():
    ...
    mod = MLEModel(
        ...
        k_states=1,
        initialization="approximate_diffuse",
        initial_variance=1000000000000000.0,
