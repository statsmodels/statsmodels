...
from statsmodels.tsa.statespace.tbats import (
    TBATSModel)
...
class CheckTBATSModel:
    ...
    def test_fitted(self):
        ...
        assert_allclose(self.res.fittedvalues, predicted.iloc[:self.nobs], rtol=1e-4, atol=1e-3)
    ...
    def test_output(self):
        ...
        if has_llf:
            assert_allclose(self.res.mse, results_params[self.name]['mse'], rtol=1e-4, atol=1e-3)
            ...
            assert_allclose(actual, results_params[self.name]['llf'], rtol=1e-4, atol=1e-3)
        else:
            assert_allclose(self.res.sse, results_params[self.name]['sse'], rtol=1e-4, atol=1e-3)
    ...
    def test_conf_int(self):
        ...
        assert_allclose(ci_95['lower y'], lower.iloc[self.nobs:], rtol=1e-4, atol=1e-3)
        assert_allclose(ci_95['upper y'], upper.iloc[self.nobs:], rtol=1e-4, atol=1e-3)

    def test_initial_states(self):
        mask = results_states.columns.str.startswith(self.name)
        desired = results_states.loc[:, mask].dropna().iloc[0]
        assert_allclose(self.res.initial_state.iloc[0], desired, rtol=1e-4, atol=1e-3)

    def test_states(self):
        mask = results_states.columns.str.startswith(self.name)
        desired = results_states.loc[:, mask].dropna().iloc[1:]
        assert_allclose(self.res.filtered_state[1:].T, desired, rtol=1e-4, atol=1e-3)
...
# class TestSESFPPEstimated(CheckTBATSModel):
#     @classmethod
#     def setup_class(cls):
#         # Test simple exponential smoothing (FPP: 7.1) against fpp::ses, with
#         # estimated coefficients
#         mod = TBATSModel(oildata, initialization_method='estimated',
...
# class TestSESETSEstimated(CheckTBATSModel):
#     @classmethod
#     def setup_class(cls):
...
class TestHoltDampedFPPEstimated(CheckTBATSModel):
    @classmethod
    def setup_class(cls):
        # Test Holt's linear trend method (FPP: 7.2) with a damped trend
        # against fpp::holt, with estimated coefficients

        mod = TBATSModel(air, trend=True, damped_trend=True,
                                   concentrate_scale=False)
        params = [results_params['air_fpp2']['sigma2'],
...
class TestHoltDampedETSEstimated(CheckTBATSModel):
    @classmethod
    def setup_class(cls):
        # Test Holt's linear trend method (FPP: 7.2) with a damped trend
        ...
        mod = TBATSModel(air, trend=True, damped_trend=True,
                                   concentrate_scale=False)
        params = [results_params['air_ets']['sigma2']
...
# class TestHoltWintersFPPEstimated(CheckTBATSModel):
#     @classmethod
#     def setup_class(cls):
#         # Test Holt-Winters seasonal method (FPP: 7.5) against fpp::hw,
#         # with estimated coefficients

#         mod = TBATSModel(aust, trend=True, seasonal=4,
#                                    concentrate_scale=False)
#         params = np.r_[
...
# class TestHoltWintersETSEstimated(CheckTBATSModel):
#     @classmethod
#     def setup_class(cls):
...
# class TestHoltWintersDampedETSEstimated(CheckTBATSModel):
#     @classmethod
#     def setup_class(cls):
#         # Test Holt-Winters seasonal method (FPP: 7.5) with a damped trend
#         # against forecast::ets, with estimated coefficients

#         mod = TBATSModel(aust, trend=True, damped_trend=True,
...
# class TestHoltWintersNoTrendETSEstimated(CheckTBATSModel):
#     @classmethod
#     def setup_class(cls):
#         # Test Holt-Winters seasonal method (FPP: 7.5) with no trend
#         # against forecast::ets, with estimated coefficients

#         mod = TBATSModel(aust, seasonal=4, concentrate_scale=False)
#         params = np.r_[
...
#     @classmethod
#     def setup_class(cls):
#         mod = TBATSModel(aust, seasonal=4)
#         start_params = pd.Series(
...
#     @classmethod
#     def setup_class(cls):
#         mod = TBATSModel(oildata, initialization_method='heuristic')
...
#     mod1 = TBATSModel(oildata, initialization_method='concentrated')
#     mod2 = TBATSModel(oildata)
...
# class TestMultiIndex(CheckTBATSModel):
#     @classmethod
#     def setup_class(cls):
#         oildata_copy = oildata.copy()
#         oildata_copy.name = ("oil", "data")
#         mod = TBATSModel(oildata_copy,
#                                    initialization_method='simple')
#         res = mod.filter([results_params['oil_fpp2']['alpha']])
...
#                        match='TBATSModel does not support `exog`.'):
#         mod = TBATSModel(aust)
...
#     ses = TBATSModel(x, initial_level=x[0],
