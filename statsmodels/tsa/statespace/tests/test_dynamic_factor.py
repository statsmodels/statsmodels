...
class CheckDynamicFactor(object):
    ...
    def test_params(self):
        ...
        untransformed = model.untransform_params(model.start_params)
        actual = model.transform_params(untransformed)
    ...
    def test_results(self, close_figures):
        ...
        if self.model.factor_order > 0:
            model = self.model
            fparams = self.results.params[model._params_factor_transition]
            fparams = np.array(fparams)
            coefficients = fparams.reshape(model.k_factors,
                                           model.k_factors*model.factor_order)
            coefficient_matrices = np.array([
                coefficients[:model.k_factors,
                             i*model.k_factors:(i+1)*model.k_factors]
                for i in range(model.factor_order)
            ])
            assert_equal(self.results.coefficient_matrices_var,
                         coefficient_matrices)
        else:
            assert self.results.coefficient_matrices_var is None
    ...
    @pytest.mark.xfail(reason="No idea!  Just xfailing so as to not "
                              "silently skip")
...
class TestDynamicFactor(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_dfm_1',
                                                   'predict_dfm_2',
                                                   'predict_dfm_3']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_dfm_1', 'dyn_predict_dfm_2', 'dyn_predict_dfm_3']]
        super(TestDynamicFactor, cls).setup_class(true, k_factors=1,
                                                  factor_order=2)
...
class TestDynamicFactor2(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_dfm2_1',
                                                   'predict_dfm2_2',
                                                   'predict_dfm2_3']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_dfm2_1', 'dyn_predict_dfm2_2', 'dyn_predict_dfm2_3']]
        super(TestDynamicFactor2, cls).setup_class(true, k_factors=2,
                                                   factor_order=1)
    ...
    def test_summary(self):
        ...
        assert len(tables) == 2 + self.model.k_endog + self.model.k_factors + 1

        # Check the model overview table
        assert re.search(r'Model:.*DynamicFactor\(factors=2, order=1\)',
                         tables[0])
        ...
        for i in range(self.model.k_endog):
            re.search(
                'sigma2.%s +%s' % (self.model.endog_names[i],
                                   forg(params[offset + i], prec=4)),
                table)
...
class TestDynamicFactor_exog1(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_dfm_exog1_1',
                                                   'predict_dfm_exog1_2',
                                                   'predict_dfm_exog1_3']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_dfm_exog1_1', 'dyn_predict_dfm_exog1_2',
            'dyn_predict_dfm_exog1_3']]
        exog = np.ones((75, 1))
        super(TestDynamicFactor_exog1,
              cls).setup_class(true, k_factors=1, factor_order=1, exog=exog)
...
class TestDynamicFactor_exog2(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_dfm_exog2_1',
                                                   'predict_dfm_exog2_2',
                                                   'predict_dfm_exog2_3']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_dfm_exog2_1', 'dyn_predict_dfm_exog2_2',
            'dyn_predict_dfm_exog2_3']]
        exog = np.c_[np.ones((75, 1)), (np.arange(75) + 2)[:, np.newaxis]]
        super(TestDynamicFactor_exog2,
              cls).setup_class(true, k_factors=1, factor_order=1, exog=exog)
    ...
    def test_summary(self):
        ...
        assert re.search(r'Model:.*DynamicFactor\(factors=1, order=1\)',
                         tables[0])
        assert re.search(r'.*2 regressors', tables[0])
...
class TestDynamicFactor_general_errors(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_dfm_gen_1',
                                                   'predict_dfm_gen_2',
                                                   'predict_dfm_gen_3']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_dfm_gen_1', 'dyn_predict_dfm_gen_2',
            'dyn_predict_dfm_gen_3']]
        super(TestDynamicFactor_general_errors,
              cls).setup_class(true, k_factors=1, factor_order=1,
                               error_var=True, error_order=1,
    ...
    def test_summary(self):
        ...
        # Make sure we have the right number of tables
        assert len(tables) == (2 + self.model.k_endog +
                               self.model.k_factors + self.model.k_endog + 1)
...
class TestDynamicFactor_ar2_errors(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_dfm_ar2_1',
                                                   'predict_dfm_ar2_2',
                                                   'predict_dfm_ar2_3']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_dfm_ar2_1', 'dyn_predict_dfm_ar2_2',
            'dyn_predict_dfm_ar2_3']]
        super(TestDynamicFactor_ar2_errors,
              cls).setup_class(true, k_factors=1,
                               factor_order=1, error_order=2)
    ...
    def test_mle(self):
        with warnings.catch_warnings(record=True):
            ...
            res = mod.fit(res1.params, method='nm', maxiter=10000,
...
class TestDynamicFactor_scalar_error(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_dfm_scalar_1',
                                                   'predict_dfm_scalar_2',
...
class TestStaticFactor(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_sfm_1',
                                                   'predict_sfm_2',
                                                   'predict_sfm_3']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_sfm_1', 'dyn_predict_sfm_2',
            'dyn_predict_sfm_3']]
        super(TestStaticFactor, cls).setup_class(true, k_factors=1,
                                                 factor_order=0)
...
class TestSUR(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_sur_1',
                                                   'predict_sur_2',
                                                   'predict_sur_3']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_sur_1', 'dyn_predict_sur_2',
            'dyn_predict_sur_3']]
        exog = np.c_[np.ones((75, 1)), (np.arange(75) + 2)[:, np.newaxis]]
        super(TestSUR, cls).setup_class(true, k_factors=0, factor_order=0,
                                        exog=exog,
                                        error_cov_type='unstructured')
...
class TestSUR_autocorrelated_errors(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true['predict'] = output_results.iloc[1:][['predict_sur_auto_1',
                                                   'predict_sur_auto_2']]
        true['dynamic_predict'] = output_results.iloc[1:][[
            'dyn_predict_sur_auto_1', 'dyn_predict_sur_auto_2']]
        exog = np.c_[np.ones((75, 1)), (np.arange(75) + 2)[:, np.newaxis]]
        super(TestSUR_autocorrelated_errors,
              cls).setup_class(true, k_factors=0, factor_order=0,
                               exog=exog, error_order=1, error_var=True,
...
def test_misspecification():
    ...
    with pytest.raises(ValueError):
        dynamic_factor.DynamicFactor(endog, k_factors=1, factor_order=1,
...
def test_miscellaneous():
    ...
    exog = pd.Series(np.arange(75),
                     index=pd.date_range(start='1960-04-01', end='1978-10-01',
                                         freq='QS'))
    mod = CheckDynamicFactor()
    mod.setup_class(true=None, k_factors=1, factor_order=1,
                    exog=exog, filter=False)
