...
def test_structural():
    ...
    mod = structural.UnobservedComponents(
        [0], "irregular", seasonal=2, stochastic_seasonal=False
    )
    actual = mod.impulse_responses([1.0], steps)
    assert_allclose(actual, 0)
    ...
    # Cycle (stochastic)
    mod = structural.UnobservedComponents(
        [0], "irregular", cycle=True, stochastic_cycle=True
    )
...
def test_varmax():
    ...
    rs = np.random.RandomState(2867613)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        mod = varmax.VARMAX(
            rs.normal(size=(steps, 2)),
            order=(2, 2),
            trend="c",
            exog=np.ones(steps),
...
def test_dynamic_factor():
    rs = np.random.RandomState(2867611)
    steps = 10
    exog = rs.normal(size=steps)
...
class TVSS(mlemodel.MLEModel):
    ...
    Uses a fixed RandomState. Modification needed for more general purpose use.
    """
    ...
    def __init__(self, endog, _k_states=None):
        ...
        if _k_states is None:
            _k_states = k_states
        super().__init__(
            endog, k_states=_k_states, k_posdef=k_posdef, initialization="diffuse"
        )
        rs = np.random.RandomState(2867611)
        self["obs_intercept"] = rs.normal(size=(self.k_endog, self.nobs))
        ...
        self["design", :, :k_states, :] = rs.normal(
        ...
        D = [np.diag(d) for d in rs.uniform(-1.1, 1.1, size=(self.nobs, k_states))]
        Q = ortho_group.rvs(k_states, size=self.nobs, random_state=rs)
        self["transition", :k_states, :k_states, :] = (
            Q @ D @ Q.transpose(0, 2, 1)
        ).transpose(1, 2, 0)
        self["selection", :k_states, :, :] = rs.normal(
            size=(k_states, self.ssm.k_posdef, self.nobs)
        )

        # Need to make sure the covariances are positive definite
        H05 = rs.normal(size=(self.k_endog, self.k_endog, self.nobs))
        Q05 = rs.normal(size=(self.ssm.k_posdef, self.ssm.k_posdef, self.nobs))
        H = np.zeros_like(H05)
        Q = np.zeros_like(Q05)
...
def test_time_varying_in_sample():
    ...
    coirfs = mod.impulse_responses(
        [], steps=mod.nobs - 1, cumulative=True, orthogonalized=True
    )
...
def test_time_varying_out_of_sample():
    rs = np.random.RandomState(2867611)
    mod = TVSS(np.zeros((10, 2)))

    # Compute all in-sample IRFs and also one out-of-sample IRF
    new_Z = rs.normal(size=mod["design", :, :, -1].shape)
    new_T = rs.normal(size=mod["transition", :, :, -1].shape)
...
def test_time_varying_in_sample_anchored():
    mod = TVSS(np.zeros((10, 2)))
    ...
    # Cumulative IRFs
    cirfs = mod.impulse_responses(
        [], steps=mod.nobs - 1 - anchor, anchor=anchor, cumulative=True
    )
    # Orthogonalized IRFs
    oirfs = mod.impulse_responses(
...
def test_time_varying_out_of_sample_anchored():
    mod = TVSS(np.zeros((10, 2)))
...
def test_time_varying_out_of_sample_anchored_end():
    rs = np.random.RandomState(2867611)
    mod = TVSS(np.zeros((10, 2)))
    ...
    new_Z = rs.normal(size=mod["design", :, :, -2:].shape)
    new_T = rs.normal(size=mod["transition", :, :, -2:].shape)
    irfs = mod.impulse_responses(
