...
def test_smoothed_decomposition_sarimax(
...
def test_smoothed_decomposition_TVSS(univariate):
    endog = np.zeros((10, 3))
    endog[6, 0] = np.nan
    endog[7, :] = np.nan
    endog[8, 1] = np.nan
    ...
    rs = np.random.RandomState(9991613)
    mod["state_intercept"] = rs.normal(size=(mod.k_states, mod.nobs))
