...
def parse(path, ssm):
    ...
    res["forecasts_error_cov"] = np.c_[
        [np.diag(x) for y, x in dta["F"].iloc[:n].iterrows()]
    ].T
    ...
    res["predicted_diffuse_state_cov"] = (
        dta["Pinf"].values.reshape(n + 1, m, m, order="F").T
    )
    # loglike
    ...
    res["smoothed_state_disturbance_cov"] = (
        dta["V_eta"].values[:n].reshape(n, r, r, order="F").T
    )
