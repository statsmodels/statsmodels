...
def matlab_results():
    ...
    def get_data(us_data, mean_M=None, std_M=None, mean_Q=None, std_Q=None):
        dta_M = us_data[
            ["CPIAUCSL", "UNRATE", "PAYEMS", "RSAFS", "TTLCONS", "TCU"]
        ].copy()
...
def test_known(matlab_results, run):
    ...
    assert_allclose(
        res.filter_results.smoothed_forecasts.T[1:], results["smoothed_forecasts"][:-1]
    )
    assert_allclose(
...
def test_emstep1(matlab_results, run):
    ...
    res2, params2 = mod._em_iteration(
        results1["params"], init=init, mstep_method="missing"
    )
    ...
    assert_allclose(
        params2[mod._p["idiosyncratic_ar1"]], true2[mod._p["idiosyncratic_ar1"]]
    )
    assert_allclose(
        params2[mod._p["idiosyncratic_var"]], true2[mod._p["idiosyncratic_var"]]
    )
...
@pytest.mark.parametrize(
    "run", ["news_112", "news_222", "news_block_112", "news_block_222"]
)
def test_news(matlab_results, run):
    ...
    assert_allclose(
        news.revision_impacts.loc["2016-09", "GDPC1"], results["revision_impacts"]
    )
