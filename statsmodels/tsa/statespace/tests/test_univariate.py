...
class TestClark1989:
    ...
    def setup_class(cls, dtype=float, alternate_timing=False, **kwargs):
        ...
        n_disturbance_variates = (
            cls.model.k_endog + cls.model.k_posdef
        ) * cls.model.nobs
    ...
    def test_using_univariate(self):
        ...
        assert_allclose(
            self.conventional_results.forecasts_error_cov[1, 1, 0], 143.03724478030821
        )
        assert_allclose(
            self.univariate_results.forecasts_error_cov[1, 1, 0], 120.66208525029386
        )

    def test_forecasts(self):
        assert_almost_equal(
            self.conventional_results.forecasts[0, :],
            self.univariate_results.forecasts[0, :],
            9,
        )
    ...
    def test_filtered_state(self):
        assert_almost_equal(
            ...
            self.univariate_results.filtered_state,
            8,
        )
    ...
    def test_predicted_state(self):
        assert_almost_equal(
            ...
            self.univariate_results.predicted_state,
            8,
        )
    ...
    def test_loglike(self):
        assert_allclose(
            self.conventional_results.llf_obs, self.univariate_results.llf_obs
        )
    ...
    def test_simulation_smoothed_state(self):
        assert_almost_equal(
            ...
            self.univariate_sim.simulated_state,
            9,
        )
...
class MultivariateMissingGeneralObsCov:
    ...
    def setup_class(cls, which, dtype=float, alternate_timing=False, **kwargs):
        # Results
        path = os.path.join(
            current_path, "results", "results_smoothing_generalobscov_R.csv"
        )
    ...
    def test_using_univariate(self):
        ...
        assert_allclose(
            self.conventional_results.forecasts_error_cov[1, 1, 0], 1000000.77
        )
        assert_allclose(
            self.univariate_results.forecasts_error_cov[1, 1, 0], 1000000.77
        )
...
def test_time_varying_transition():
    ...
    assert_allclose(
        res1.forecasts_error_cov[0, 0, :], res2.forecasts_error_cov[0, 0, :]
    )
    ...
    assert_allclose(
        res1.smoothed_measurement_disturbance, res2.smoothed_measurement_disturbance
    )
    assert_allclose(
    ...
    assert_allclose(
        res1.smoothed_state_disturbance_cov, res2.smoothed_state_disturbance_cov
    )

    assert_allclose(sim1.simulated_state, sim2.simulated_state)
    assert_allclose(
        sim1.simulated_measurement_disturbance, sim2.simulated_measurement_disturbance
    )
