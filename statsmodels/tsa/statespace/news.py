...
class NewsResults:
    ...
    def __init__(
        ...
        self.revisions_iloc = pd.DataFrame(
            ...
            index=["revision date", "revised variable"],
        ).T
        ...
        self.updates_iloc = pd.DataFrame(
            ...
            index=["update date", "updated variable"],
        ).T
        ...
        if news_results.revisions is None:
            self.revisions = pd.Series(
                [], index=r_ix, name="revision", dtype=model.params.dtype
            )
        else:
            self.revisions = pd.Series(
                news_results.revisions, index=r_ix, name="revision"
            )
        # E[y^u | revised]
        ...
        if news_results.revised_all is None:
            ...
            self.revised_all = pd.Series(
                news_results.revised_all, index=r_ix_all, name="revised"
            )
        # y^r_{revised} for which detailed impacts were computed
        if news_results.revised is None:
            self.revised = pd.Series(
                [], index=r_ix, dtype=model.params.dtype, name="revised"
            )
        else:
        ...
        if news_results.revised_prev_all is None:
            self.revised_prev_all = pd.Series(
                [], index=r_ix_all, dtype=model.params.dtype
            )
        else:
            self.revised_prev_all = pd.Series(
        ...
        self.revision_weights = pd.DataFrame(
            revision_weights, index=cols, columns=r_ix
        ).T
    ...
    def data_revisions(self):
        ...
        data["detailed impacts computed"] = self.revised_all.index.isin(
            self.revised.index
        )
        return data
    ...
    def _revision_grouped_impacts(self):
        ...
        if self.revisions_details_start > 0:
            df["revision date"] = self.updated.model._index[
                self.revisions_details_start - 1
            ]
            df["revised variable"] = "all prior revisions"
        df = df.set_index(
            ["revision date", "revised variable"], append=True
    ...
    def impacts(self):
        ...
        impacts["total impact"] = (
            impacts["impact of revisions"] + impacts["impact of news"]
        )
    ...
    def summary_details(
        ...
        if source == "news":
            ...
            raise ValueError(
                f'Invalid `source`: {source}. Must be "news" or' ' "revisions".'
            )
        ...
        details.index = details.index.reorder_levels(
            levels_order
        ...
        if groupby_overall == "update":
            details.set_index(
                [columns["current"], columns["prev"]], append=True, inplace=True
            )
                ...
                if k_level_values[i] == 1:
                    ...
                    can_drop = (
                        ...
                        or (
                            ...
                            and updated_variable is not None
                        )
                        or (name == "impact date" and impact_date is not None)
                        or (
                            name == "impacted variable"
                            and (
                                impacted_variable is not None
                                or self.impacted_variable is not None
                            )
                        )
                    )
    ...
    def get_details(self, include_revisions=True, include_updates=True):
        ...
        if include_updates:
            details.append(
                self.details_by_impact.rename(columns={"forecast (prev)": "previous"})
            )
        if include_revisions:
            tmp = self.revision_details_by_impact.rename_axis(
                index={
                    "revision date": "update date",
