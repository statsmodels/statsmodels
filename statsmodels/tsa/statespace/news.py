...
class NewsResults(object):
    ...
    def data_updates(self):
        ...
        data_updates
        """
        data = pd.concat([self.update_forecasts, self.update_realized],
    ...
    def details_by_update(self):
        ...
        df = df[['forecast (prev)', 'observed', 'news',
                 'weight', 'impact']]
        df = df.reset_index()
        keys = ['update date', 'updated variable', 'forecast (prev)',
    ...
    def summary_impacts(self, impact_date=None, impacted_variable=None,
                        groupby='impact date', show_revisions=None,
    ...
    def summary_details(self, impact_date=None, impacted_variable=None,
        ...
        if groupby_overall == 'update':
            details.set_index(['forecast (prev)', 'observed'], append=True,
                              inplace=True)
            details.index = details.index.reorder_levels([0, 1, 4, 5, 2, 3])
            base_levels = [0, 1, 4, 5]

        # Drop the non-groupby levels if there's only one value
        n_levels = len(details.index.levels)
        k_level_values = [len(details.index.levels[i])
                          for i in range(n_levels)]
        removed_levels = []
        if sparsify:
            for i in sorted(base_levels)[::-1][:-1]:
                if k_level_values[i] == 1:
                    name = details.index.names[i]
                    value = details.index.levels[i][0]
                    can_drop = (
                    ...
                    if can_drop or not multiple_tables:
                        ...
                        details.index = details.index.droplevel(i)
        ...
        def str_format(num, mark_ones=False, mark_zeroes=False,
            ...
            if pd.isnull(num):
                out = ''
            elif mark_ones and np.abs(1 - num) < tolerance:
                out = '1.0'
            elif mark_zeroes and np.abs(num) < tolerance:
                out = '0'
            else:
        ...
        def create_table(details, removed_levels):
            ...
            for key in ['forecast (prev)', 'observed', 'news', 'weight',
                        'impact']:
                if key in details:
                    args = (
                        # mark_ones
            ...
            if sparsify:
                ...
                if groupby_overall == 'update':
                    sparsify_cols += ['forecast (prev)', 'observed']
    ...
    def summary(self, impact_date=None, impacted_variable=None,
                ...
                show_revisions=None, sparsify=True, include_details=None,
                include_data_updates=True, float_format='%.2f'):
        """
        ...
        include_data_updates : bool, optional
            If set to False, the summary will hide tables describing the
        ...
        summary_tables : Summary
            ...
            - Tables describing changes in the dataset (revisions and updates)
              since the previous results set (unless
              `include_data_updates=False`).

            In univariate models or if `include_details=True`, one or more
            tables will additionally be included describing the details of how
            news from specific updates translate into specific impacts.
        ...
        summary_updates
        """
        # Default for include_details
        if include_details is None:
            include_details = self.updated.model.k_endog == 1
        ...
        summary.tables.insert(table_ix, self.summary_impacts(
            ...
            groupby=impacts_groupby, show_revisions=show_revisions,
            sparsify=sparsify, float_format=float_format))
        table_ix += 1
...
class NewsResultsWrapper(wrap.ResultsWrapper):
    _attrs = {}
    _wrap_attrs = wrap.union_dicts(_attrs)

    _methods = {}
    _wrap_methods = wrap.union_dicts(_methods)
wrap.populate_wrapper(NewsResultsWrapper, NewsResults)  # noqa:E305
