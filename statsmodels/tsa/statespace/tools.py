...
def _compute_obs_weights_smoothed_state(res, compute_t=None, compute_j=None,
                                        refilter=True):
    # Get references to model etc.
    mod = res.model
    if refilter:
        # Below, we need the tmp3 and kalman_gain matrices in the format that
        # they exist in the Cython objects (i.e. with only the elements
        # corresponding to the non-missing observations retained, and those
        # grouped together in the first entries).
        # However, we cannot guarantee that the _kalman_filter object that
        # holds those matrices has not changed since `res` was created. As a
        # result, by default we re-run the filter to get those matrices in the
        # correct order, although this can be overridden by a user that is
        # sure that the model has not changed in-between times
        mod.filter(res.params, conserve_memory=0, return_ssm=True)
    _ss = mod.ssm._statespace
    _kf = mod.ssm._kalman_filter
    _ks = mod.ssm._kalman_smoother
    sres = res.smoother_results
    ...
    Hi_W = np.zeros((n, p, m)) * np.nan
    K = []
    weights = np.zeros((n, n, m, p)) * np.nan
    I = np.eye(m)
    nmissing = np.array(mod.ssm._statespace.nmissing)

    # First pass to compute required quantities
    for t in range(n):
        p_t = p - nmissing[t]

        # Handling of exact diffuse periods is not currently available.
        if t < res.nobs_diffuse:
            K.append(np.zeros((m, p_t)) * np.nan)
        ...
        transition_t = t if _ss.transition.shape[2] > 1 else 0
        T = np.array(_ss.transition[..., transition_t])
        ...
        if p_t == 0:
            ...
            Z = (np.array(_ss.selected_design)[:p_t * m]
                   .reshape(p_t, m, order='F'))
            H = (np.array(_ss.selected_obs_cov)[:p_t * p_t]
                   .reshape(p_t, p_t, order='F'))
        else:
            design_t = t if _ss.design.shape[2] > 1 else 0
            Z = np.array(_ss.design[..., design_t])
            obs_cov_t = t if _ss.obs_cov.shape[2] > 1 else 0
            H = np.array(_ss.obs_cov[..., obs_cov_t])

        # Get filtered objects
        FILTER_COLLAPSED = 0x20
        if (sres.univariate_filter[t] or
                (sres.filter_method & FILTER_COLLAPSED)):
            P = np.array(sres.predicted_state_cov[..., t])
            F = Z @ P @ Z.T + H
            ...
            FiZ = np.array(_kf.tmp3[..., t])[:p_t] / res.scale
            K.append(np.array(_kf.kalman_gain[..., t])[:, :p_t])

        # Get smoothed objects
        L = np.array(sres.innovations_transition[..., t])
        N = np.array(sres.scaled_smoothed_estimator_cov[..., t])

        # Compute required quantites
        # W_j = H_j (F_j^{-1} Z_j - K_j' N_j L_j)
        # => H_j^{-1} W_j = F_j^{-1} Z_j - K_j' N_j L_j
        Hi_W[t, :p_t] = FiZ - K[t].T @ N @ L

    # Second pass to compute weights
    min_j = np.min(compute_j)
    max_j = np.max(compute_j)
    for t in compute_t:
        if t < res.nobs_diffuse:
            continue

        Pt = np.array(sres.predicted_state_cov[..., t])
        if t == 0:
            Nt1 = np.array(sres.scaled_smoothed_estimator_cov_presample)
        else:
            Nt1 = np.array(sres.scaled_smoothed_estimator_cov[..., t - 1])

        # Backwards from j = t-1, t-2, ... min_j
        tmp = (I - Pt @ Nt1)
        for j in range(min_j, t)[::-1]:
            p_j = p - nmissing[j]
            L = np.array(sres.innovations_transition[..., j])
        ...
        p_j = p - nmissing[t]
        if j in compute_j:
            weights[t, t, :, :p_j] = Pt @ Hi_W[j, :p_j].T

        # Forwards from j = t+1, t+2, ..., max_j
        tmp = Pt
        for j in range(t + 1, max_j + 1):
            p_j = p - nmissing[j]
            tmp = tmp @ np.array(sres.innovations_transition[..., j - 1]).T

            if j in compute_j:
                weights[t, j, :, :p_j] = tmp @ Hi_W[j, :p_j].T

    # Re-order missing entries correctly
    if np.any(nmissing):
        shape = weights.shape
        weights = np.asfortranarray(weights.transpose(0, 2, 3, 1).reshape(
        ...
        reorder_missing_matrix(weights, missing, reorder_cols=True,
                               inplace=True)
        weights = (weights.reshape(shape[0], shape[2], shape[3], shape[1])
                          .transpose(0, 3, 1, 2))

    return weights, Hi_W


def compute_obs_weights_smoothed_state_python(res, compute_t=None,
                                              compute_j=None, resmooth=True):
    r"""
    ...
    """
    out = _compute_obs_weights_smoothed_state(
        res, compute_t=compute_t, compute_j=compute_j, resmooth=resmooth)
    return out[0]
