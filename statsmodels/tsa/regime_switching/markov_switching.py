...
def py_hamilton_filter(initial_probabilities, regime_transition,
    ...
    """
    Hamilton filter using pure Python. Historial reference only.
    ...
    joint_loglikelihoods : array
        Array of loglikelihoods condition on time t information, shaped (nobs,).
    ...
    for i in range(order):
        t = i if regime_transition.shape[-1] > 1 else 0
        tmp = np.reshape(regime_transition[..., t], shape + (1,) * i) * tmp
...
def py_hamilton_filter_log(initial_probabilities, regime_transition,
                           conditional_loglikelihoods):
    """
    Hamilton filter in log space using pure Python. Historial reference only.
    ...
    # Check that regime_transition is oriented correctly.
    if not np.allclose(np.sum(np.exp(regime_transition), axis=0), 1):
        raise ValueError('regime_transition does not contain '
                         'left stochastic matrices.')
    ...
    for t in range(nobs):
        ...
        if order > 0:
            predicted_joint_probabilities[..., t] = (
                ...
                regime_transition[..., transition_t] +
                # S_{t-1}, S_{t-2}, ..., S_{t-r} | t-1
                logsumexp(filtered_joint_probabilities[..., t], axis=-1))
        else:
            predicted_joint_probabilities[..., t] = np.log(
                np.dot(regime_transition[..., transition_t],
                       np.exp(filtered_joint_probabilities[..., t])))

        # f(y_t, S_t, ..., S_{t-r} | t-1)
        tmp = (conditional_loglikelihoods[..., t] +
               predicted_joint_probabilities[..., t])
        # f(y_t | t-1)
        joint_likelihoods[t] = logsumexp(tmp)
    ...
    filtered_marginal_probabilities = filtered_joint_probabilities[..., 1:]
    for i in range(1, filtered_marginal_probabilities.ndim - 1):
        filtered_marginal_probabilities = logsumexp(
            filtered_marginal_probabilities, axis=-2)

    return (np.exp(filtered_marginal_probabilities), np.exp(predicted_joint_probabilities),
            joint_likelihoods, np.exp((filtered_joint_probabilities[..., 1:])))


def cy_hamilton_filter(initial_probabilities, regime_transition,
                       conditional_likelihoods):
    """
    Hamilton filter using Cython inner loop. Historial reference only.
...
def py_kim_smoother(regime_transition, predicted_joint_probabilities,
    ...
    """
    Kim smoother using pure Python. Historial reference only.
...
def cy_kim_smoother(regime_transition, predicted_joint_probabilities,
                    filtered_joint_probabilities):
    """
    Kim smoother using Cython inner loop. Historial reference only.
