...
class TestFedFundsConstShort(MarkovRegression):
    ...
    def test_smoother_output(self, **kwargs):
        ...
        # Smoothed, last entry
        assert_allclose(
            res.smoothed_joint_probabilities,
...
class TestMumpspcNoconstL1Variance(MarkovRegression):
    ...
    def setup_class(cls):
        true = {
            "params": np.r_[
                0.762733, 0.1473767, 0.420275, 0.9847369, 0.0562405**2, 0.2611362**2
            ],
            "llf": 131.7225,
            "llf_fit": 131.7225,
            "llf_fit_em": 131.7175,
...
def test_exog_tvtp():
    ...
    params_tvtp[0] = np.squeeze(
        mod2._untransform_logistic(np.r_[0.0], np.r_[1 - params[0]])
    )
    params_tvtp[1] = np.squeeze(
        mod2._untransform_logistic(np.r_[0.0], np.r_[1 - params[1]])
    )
    ...
    assert_allclose(
        res2.predicted_joint_probabilities, res1.predicted_joint_probabilities
    )
    assert_allclose(
        res2.filtered_joint_probabilities, res1.filtered_joint_probabilities
    )
    assert_allclose(
