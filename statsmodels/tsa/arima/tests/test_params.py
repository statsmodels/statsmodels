...
def test_set_params_multiple():
    ...
    #     a_2 b_1 L^6 + b_2 L^8 + a_1 b_2 L^9 + a_2 b_2 L^10
    assert_equal(
        p.reduced_ma_poly.coef,
        [1, -5, 6, 0, -9, (-5 * -9), (6 * -9), 0, 10, (-5 * 10), (6 * 10)],
    )
...
def test_set_poly_short_lags():
    ...
    with pytest.raises(ValueError):
        p.__setattr__("ar_poly", [2, -0.5])
    with pytest.raises(ValueError):
        p.__setattr__("ma_poly", [2, 0.3])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ar_poly", [2, 0, 0, 0, -0.2])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ma_poly", [2, 0, 0, 0, 0.1])
    # Too short
    with pytest.raises(ValueError):
        p.__setattr__("ar_poly", 1)
    with pytest.raises(ValueError):
        p.__setattr__("ar_poly", [1])
    with pytest.raises(ValueError):
        p.__setattr__("ma_poly", 1)
    with pytest.raises(ValueError):
        p.__setattr__("ma_poly", [1])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ar_poly", 1)
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ar_poly", [1])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ar_poly", [1, 0, 0, 0])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ma_poly", 1)
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ma_poly", [1])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ma_poly", [1, 0, 0, 0])
    # Too long
    with pytest.raises(ValueError):
        p.__setattr__("ar_poly", [1, -0.5, 0.2])
    with pytest.raises(ValueError):
        p.__setattr__("ma_poly", [1, 0.3, 0.2])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ar_poly", [1, 0, 0, 0, 0.1, 0])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ma_poly", [1, 0, 0, 0, 0.1, 0])
    # Number in invalid location (only for seasonal polynomials)
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ar_poly", [1, 1, 0, 0, 0, -0.2])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ma_poly", [1, 1, 0, 0, 0, 0.1])
...
def test_set_poly_short_lags_nonconsecutive():
    ...
    with pytest.raises(ValueError):
        p.__setattr__("ma_poly", [1, 1, 0.3])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ar_poly", [1, 0, 0, 0, 1.0, 0, 0, 0, -0.2])
    with pytest.raises(ValueError):
        p.__setattr__("seasonal_ma_poly", [1, 0, 0, 0, 1.0, 0, 0, 0, 0.1])
