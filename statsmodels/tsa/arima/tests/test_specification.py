...
@pytest.mark.parametrize(
    "spec_params, fixed_params, invalid_fixed_params",
    [
        # no fixed param
        (
        ...
        (
            ...
            {"random_param": 0, "ar.L1": 0, "ar.S.L1": 0},
            ["random_param"]
        ),
        # valid fixed params
        (
        ...
        (
            ...
            {"ma.L1": 0, "ma.L2": 0, "ar.L1": 0, "ar.L2": 0},
            None
        )
    ]
)
...
@pytest.mark.parametrize(
    "spec_params, fixed_params, err_msg",
    [
        (
            {"order": (1, 1, 3)},
            {"ar.L2": 0, "ma.L2": 1},
...
def test_split_fixed_params():
    ...
    with pytest.raises(ValueError, match="Invalid fixed parameter"):
        spec.split_fixed_params({"ar.L2": 0}, validate=True)

    with pytest.raises(ValueError, match=""):
        spec.split_fixed_params({"ar.S.L5": np.inf}, validate=True)

    # valid cases
    fixed_params = {"ar.L1": 1, "ma.L2": 2, "ar.S.L5": 3, "x2": 4}

    expected_split_fixed_params = {
