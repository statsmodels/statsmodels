...
def get_index_loc(key, index):
    ...
    # Special handling for integral indices
    if (
        not range_index
        and int_index
        and not date_index
    ...
    if date_index:
        ...
        # For integral indices and RangeIndex, key is assumed to be the
        # location and not an index value (this assumption is required to
        # support RangeIndex)
        try:
            index[key]
...
def get_prediction_index(
    ...
    # kind of index...
        ...
        # Otherwise, warn the user that they will get an integral index
        else:
            if not silent:
                warnings.warn(
...
class TimeSeriesModel(base.LikelihoodModel):
    ...
    def _init_dates(self, dates=None, freq=None):
        ...
        a Pandas index, and it is always an integral index, DatetimeIndex, or
        PeriodIndex.

        If Pandas objects, endog / exog may have any type of index. If it is
        an integral index with values 0, 1, ..., nobs-1 or if it is (coerceable
        to) a DatetimeIndex or PeriodIndex *with an associated frequency*, then
        it is called a "supported" index. Otherwise it is called an
        "unsupported" index.
        ...
        Unsupported indexes are ignored, and a supported integral index is
    ...
    def _get_index_loc(self, key, base_index=None):
        ...
        -----
        If `key` is past the end of of the given index, and the index is either
        an integral index or a date index, this function extends the index up to
