...
class TestAROLSNoConstant(CheckARMixin):
    ...
    def test_mle(self):
        ...
        assert_allclose(
            res0.fittedvalues[-10:], res0.fittedvalues[-10:], rtol=0.015
        )
...
class TestARMLEConstant(object):
    ...
    def test_predict(self):
        ...
        assert_almost_equal(
            model.predict(params), self.res2.FVMLEdefault, DECIMAL_4
        )
        assert_almost_equal(
            model.predict(params, start=9, end=308),
            self.res2.FVMLEstart9end308,
            DECIMAL_4,
        )
        assert_almost_equal(
            model.predict(params, start=100, end=308),
            self.res2.FVMLEstart100end308,
            DECIMAL_4,
        )
        assert_almost_equal(
            model.predict(params, start=0, end=200),
        ...
        assert_almost_equal(
            0.5 * model.predict(params, start=200, end=333),
            0.5 * self.res2.FVMLEstart200end334,
            DECIMAL_4,
        )
        assert_almost_equal(
            0.5 * model.predict(params, start=308, end=333),
            0.5 * self.res2.FVMLEstart308end334,
            DECIMAL_4,
        )

        assert_almost_equal(
            model.predict(params, start=9, end=309),
            self.res2.FVMLEstart9end309,
            DECIMAL_4,
        )
        assert_almost_equal(
            model.predict(params, end=301),
            self.res2.FVMLEstart0end301,
            DECIMAL_4,
        )
        assert_almost_equal(
            model.predict(params, start=4, end=312),
            self.res2.FVMLEstart4end312,
            DECIMAL_4,
        )
        assert_almost_equal(
            model.predict(params, start=2, end=7),
...
def test_ar_errors(reset_randomstate):
    ...
    with pytest.raises(ValueError, match="Length of start params"):
        with pytest.warns(FutureWarning):
            AR(np.random.standard_normal(1000)).fit(
                maxlag=2, method="mle", start_params=[1, 1]
            )
