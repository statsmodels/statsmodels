...
class DeterministicTerm(ABC):
    ...
    def _extend_index(
        ...
        if isinstance(index, pd.PeriodIndex):
            ...
            return NumericIndex(idx_arr)
        # default range index
        import warnings

        warnings.warn(
            "Only PeriodIndexes, DatetimeIndexes with a frequency set, "
            "RangesIndexes, and NumericIndex with a unit increment support "
    ...
    def _range_from_range_index(self, start: int, stop: int) -> pd.DataFrame:
        ...
        if is_int64_index:
            ...
            try:
                ...
                # TODO: Remove after pandas min ver is 1.0.0+
                idx_step = index[-1] - index[-2] if len(index) > 1 else 1
        ...
        if is_int64_index:
            new_idx = NumericIndex(np.arange(start, stop))
        else:
...
RangeIndex, and NumericIndex that start at 0 and have only unit differences \
