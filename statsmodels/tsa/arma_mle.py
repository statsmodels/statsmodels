...
class Arma(GenericLikelihoodModel):
    """
    univariate Autoregressive Moving Average model, conditional on initial
    values
    ...
    In large samples conditional LS, conditional MLE and exact MLE should be
    very close to each other, they are equivalent asymptotically.
    ...
    def fit(self, order=(0, 0), start_params=None, method="ls", **optkwds):
        ...
        if method == "ls":
            ...
            def errfnsum(rho):
                return np.sum(self.geterrors(rho)**2)
    ...
    def fit_mle(self, order=(0, 0), start_params=None, method='nm',
                maxiter=5000, tol=1e-08, **kwds):
        '''
        Estimate an ARMA model with given order using Conditional Maximum
        Likelihood.

        Parameters
        ----------
        order : tuple, 2 elements
            specifies the number of lags(nar, nma) to include, not including
            lag 0
        start_params : array_like, 1d, (nar+nma+1,)
            start parameters for the optimization, the length needs to be equal
            to the number of ar plus ma coefficients plus 1 for the residual
            variance
        method : str
    ...
    def forecast2(self, step_ahead=1, start=None, end=None, endog=None):
        '''rolling h-period ahead forecast without reestimation, 1 period ahead
        only
        ...
        -----
        just the idea:
        To improve performance with expanding arrays, specify total period by
        endog and the conditional forecast period by step_ahead

        This should be used by/with results which should contain predicted
        error or noise. Could be either a recursive loop or lfilter with a
        h-step ahead forecast filter, but then I need to calculate that one.
        ???
        ...
        for i in range(start, end):
            pred = sum(arcoefs_rev*y[i-p:i]) + sum(macoefs_rev * errors[i-p:i])
    ...
    def forecast3(self, step_ahead=1, start=None): #, end=None):
        ...
        marep = arma2ma(ar, ma, start)[step_ahead+1:]
        # truncated ma representation
