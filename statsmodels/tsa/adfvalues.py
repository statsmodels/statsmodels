...
tau_small_p['nc'] = [[0.6344, 1.2378, 3.2496],
                     [1.9129, 1.3857, 3.5322],
                     [2.7648, 1.4502, 3.4186],
...
tau_small_p['c'] = [[2.1659, 1.4412, 3.8269],
...
tau_small_p['ct'] = [[3.2512, 1.6047, 4.9588],
                     [3.6646, 1.5419, 3.6448],
                     [4.0983, 1.5173, 2.9898],
                     [4.5844, 1.5338, 2.8796],
                     [5.0722, 1.5634, 2.9472],
...
tau_small_p['ctt'] = [[4.0003, 1.6580, 4.8288],
                      [4.3534, 1.6016, 3.7947],
                      [4.7343, 1.5768, 3.2396],
...
tau_large_p['nc'] = [[0.4797, 9.3557, -0.6999, 3.3066],
...
tau_large_p['c'] = [[1.7339, 9.3202, -1.2745, -1.0368],
                    [2.1945, 6.4695, -2.9198, -4.2377],
                    [2.5893, 4.5168, -3.6529, -5.0074],
                    [3.0387, 4.5452, -3.3666, -4.1921],
                    [3.5049, 5.2098, -2.9158, -3.3468],
...
tau_large_p['ct'] = [[2.5261, 6.1654, -3.7956, -6.0285],
...
tau_large_p['ctt'] = [[3.0778, 4.9529, -4.1477, -5.9359],
...
adf_z_small_p = {'nc': [.0342, -.6376, 0, -.03872],
...
def mackinnonp2(stat, regression="c", N=1, dist_type='ADF-t'):
    ...
    if N > 1 and dist_type.lower() != 'adf-t':
        raise ValueError('Cointegration results (N > 1) are' +
                         'only available for ADF-t values')
    if dist_type == 'adf-t':
        maxstat = tau_max[regression][N - 1]
        minstat = tau_min[regression][N - 1]
        starstat = tau_star[regression][N - 1]
        small_p = tau_small_p[regression][N - 1]
        large_p = tau_large_p[regression][N - 1]
...
tau_2010['nc'] = [[[-2.56574, -2.2358, -3.627, 0],  # N = 1
                   [-1.94100, -0.2686, -3.365, 31.223],
                   [-1.61682, 0.2656, -2.714, 25.364]]]
tau_2010['nc'] = asarray(tau_2010['nc'])

tau_2010['c'] = [[[-3.43035, -6.5393, -16.786, -79.433],  # N = 1, 1%
                  [-2.86154, -2.8903, -4.234, -40.040],  # 5 %
                  [-2.56677, -1.5384, -2.809, 0]],  # 10 %
                 [[-3.89644, -10.9519, -33.527, 0],  # N = 2
                  [-3.33613, -6.1101, -6.823, 0],
                 ...
                 [[-6.63790, -50.2095, -124.156, 579.622],  # N = 12
                  [-6.11279, -36.2681, -32.505, 314.802],
                  [-5.83724, -29.9864, -2.686, 184.116]]]
tau_2010['c'] = asarray(tau_2010['c'])

tau_2010['ct'] = [[[-3.95877, -9.0531, -28.428, -134.155],  # N = 1
                   [-3.41049, -4.3904, -9.036, -45.374],
                   [-3.12705, -2.5856, -3.925, -22.380]],
                  [[-4.32762, -15.4387, -35.679, 0],  # N = 2
                   [-3.78057, -9.5106, -12.074, 0],
                  ...
                  [[-6.83488, -54.7119, -139.800, 736.376],  # N = 12
                   [-6.31127, -39.9676, -37.021, 406.051],
                   [-6.03650, -33.2381, -6.606, 317.776]]]
tau_2010['ct'] = asarray(tau_2010['ct'])

tau_2010['ctt'] = [[[-4.37113, -11.5882, -35.819, -334.047],  # N = 1
                    [-3.83239, -5.9057, -12.490, -118.284],
                    [-3.55326, -3.6596, -5.293, -63.559]],
                   [[-4.69276, -20.2284, -64.919, 88.884],  # N =2
                    [-4.15387, -13.3114, -28.402, 72.741],
                   ...
                   [[-7.02582, -59.6037, -166.368, 989.879],  # N = 12
                    [-6.50353, -44.0797, -47.242, 543.889],
                    [-6.22941, -36.9673, -10.868, 418.414]]]
tau_2010['ctt'] = asarray(tau_2010['ctt'])
...
adf_z_cv_approx = {
    ...
    'ct': asarray(
        [[-29.35315571, 231.37195625, -1337.45596868, 4184.65555846],
         [-21.70997699, 129.77871533, -605.70903981, 1631.93298689],
         [-18.24398648, 93.01269869, -381.6292858, 918.50993342]])
}

def mackinnoncrit2(N=1, regression="c", nobs=inf,
                  dist_type='ADF-t'):
    """
    Returns the critical values for cointegrating and the ADF test.
    In 2010 MacKinnon updated the values of his 1994 paper with critical values
    ...
    if dist_type == 'adf-t':
        asymptotic_cv = tau_2010[regression][N - 1, :, 0]
        poly_coef = tau_2010[regression][N - 1, :, :].T
    elif dist_type == 'adf-z':
