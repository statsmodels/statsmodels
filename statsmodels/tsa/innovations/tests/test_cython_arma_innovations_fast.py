...
def test_brockwell_davis_ex533():
    ...
    unconditional_variance = (
        sigma2
        * (1 + 2 * ar_params[0] * ma_params[0] + ma_params[0] ** 2)
        / (1 - ar_params[0] ** 2)
    )
    ...
    endog = np.array(
        [-1.1, 0.514, 0.116, -0.845, 0.872, -0.467, -0.977, -1.699, -1.228, -1.093]
    )
...
def test_integrated_process(ar_params, diff, ma_params, sigma2):
    ...
    # Kalman filter apparoach
    mod = SARIMAX(
        endog, order=(len(ar_params), diff, len(ma_params)), simple_differencing=True
    )
    res = mod.filter(np.r_[ar_params, ma_params, sigma2])
