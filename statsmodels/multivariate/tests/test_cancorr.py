...
def test_cancorr():
    ...
    assert_almost_equal(
        r.stats_mv.loc["Pillai's trace", "Value"], 0.67848151, decimal=8
    )
    assert_almost_equal(
        r.stats_mv.loc["Hotelling-Lawley trace", "Value"], 1.77194146, decimal=8
    )
    assert_almost_equal(
        r.stats_mv.loc["Roy's greatest root", "Value"], 1.72473874, decimal=8
    )
    ...
    assert_almost_equal(
        r.stats_mv.loc["Hotelling-Lawley trace", "F Value"], 2.64, decimal=2
    )
    assert_almost_equal(
        r.stats_mv.loc["Roy's greatest root", "F Value"], 9.20, decimal=2
    )
    ...
    assert_almost_equal(
        r.stats_mv.loc["Hotelling-Lawley trace", "Num DF"], 9, decimal=3
    )
    ...
    assert_almost_equal(
        r.stats_mv.loc["Hotelling-Lawley trace", "Den DF"], 19.053, decimal=3
    )
    ...
    assert_almost_equal(
        r.stats_mv.loc["Hotelling-Lawley trace", "Pr > F"], 0.0357, decimal=4
    )
    assert_almost_equal(
        r.stats_mv.loc["Roy's greatest root", "Pr > F"], 0.0009, decimal=4
    )
