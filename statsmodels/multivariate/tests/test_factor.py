...
def test_example_compare_to_R_output():
    ...
    a = np.array(
        [
            [1.0283417017, 1.00178840104, 0.71824931384, -0.00013510048],
...
def test_factor_scoring():
    ...
    assert_allclose(
        f_bart * [1, -1], f_s[["f1b", "f2b"]].values, atol=0.0001, rtol=0.001
    )
    ...
    f_reg = res.factor_scoring(method="reg")
    assert_allclose(
        f_reg * [1, -1], f_s[["f1o", "f2o"]].values, atol=0.0001, rtol=0.001
    )
    f_bart = res.factor_scoring()
    assert_allclose(
        f_bart * [1, -1], f_s[["f1ob", "f2ob"]].values, atol=0.0001, rtol=0.001
    )
    f_ols = res.factor_scoring(method="ols")
    f_gls = res.factor_scoring(method="gls")
    f_reg_z = _zscore(f_reg)
