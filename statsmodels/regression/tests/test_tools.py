...
class TestMinimalWLS:
    ...
    def test_equivalence_with_wls(self, check):
        ...
        minres = _MinimalWLS(
            self.endog1, self.exog1, check_endog=check, check_weights=check
        ).fit()
        ...
        minres = _MinimalWLS(
            self.endog2, self.exog2, check_endog=check, check_weights=check
        ).fit()
        assert_allclose(res.params, minres.params)
        assert_allclose(res.resid, minres.resid)
    ...
    def test_inf_nan(self, bad_value):
        with pytest.raises(
            ValueError, match=r"detected in endog, estimation infeasible"
        ):
        ...
        with pytest.raises(
            ValueError, match=r"detected in weights, estimation infeasible"
        ):
