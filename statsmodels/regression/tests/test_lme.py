...
class TestMixedLM(object):
    ...
    @pytest.mark.slow
    @pytest.mark.parametrize('use_sqrt', [True, False])
    @pytest.mark.parametrize('reml', [True, False])
    @pytest.mark.parametrize('profile_fe', [True, False])
    def test_compare_numdiff(self, use_sqrt, reml, profile_fe):
        ...
        for kr in range(5):
            ...
            cov_re = np.dot(cov_re.T, cov_re).round(8)
            vcomp = (np.random.normal(size=2)**2).round(8)
    ...
    def test_default_re(self):
        ...
        assert_allclose(mdf1.params, mdf2.params, rtol=1.5e-8)
    ...
    def test_formulas(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            rslt5 = mod5.fit()
        assert_allclose(rslt4.params, rslt5.params)
...
def do1(reml, irf, ds_ix):
    ...
    assert_allclose(mdf.fe_params, rslt.coef, rtol=1.5e-4)
    assert_allclose(mdf.cov_re, rslt.cov_re_r, rtol=1.5e-4)
    assert_allclose(mdf.scale, rslt.scale_r, rtol=1.5e-4)

    k_fe = md.k_fe
    assert_allclose(
        rslt.vcov_r, mdf.cov_params()[0:k_fe, 0:k_fe], rtol=1.5e-3)

    assert_allclose(mdf.llf, rslt.loglike[0], rtol=1.5e-2)

    # Not supported in R except for independent random effects
    if not irf:
        assert_allclose(
            mdf.random_effects[0], rslt.ranef_postmean, rtol=1.5e-3)
        assert_allclose(
            mdf.random_effects_cov[0], rslt.ranef_condvar, rtol=1.5e-3)
