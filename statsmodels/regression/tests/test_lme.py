...
class R_Results:
    ...
    def __init__(self, meth, irfs, ds_ix):
        ...
        fname = Path(rdir).joinpath("lme%02d.csv" % ds_ix)
...
class TestMixedLM:
    ...
    def test_vcomp_2(self):
        ...
        assert_allclose(
            result1.params["x1"], 2 * result1_env.params["times_two(x1)"], rtol=0.001
        )
    ...
    def test_dietox_slopes(self):
        ...
        assert_allclose(
            ...
            np.r_[19.4934552, 0.2938323, 0.2938323, 0.416062],
            rtol=0.1,
        )
...
cur_dir = Path(__file__).resolve().parent
rdir = Path(cur_dir).joinpath("results")
fnames = [p.name for p in Path(rdir).iterdir()]
...
def test_singular():
    ...
    with pytest.warns(SingularMatrixWarning, match="effects"):
        mdf = md.fit()
    mdf.summary()
