...
class RecursiveLSResults(MLEResults):
    ...
    def plot_recursive_coefficient(self, variables=0, alpha=0.05,
        ...
        filtered_variables = []
        filtered_variable_covs = []
        variable_labels = []

        coef = self.recursive_coefficients

        for variable in variables:
            filtered_variables.append(coef.filtered[variable, :])
            filtered_variable_covs.append(coef.filtered_cov[variable, variable,
                    :])
            variable_labels.append(
                    'Recursive estimates: %s' % exog_names[variable])

        filtered_variables = np.asarray(filtered_variables)
        filtered_variable_covs = np.asarray(filtered_variable_covs)

        return self._plot_values(filtered_variables, filtered_variable_covs,
                variable_labels, alpha, legend_loc, fig, figsize)
