...
* `cov_re` is the random effects covariance matrix (referred to above
  as Psi) and `scale2` is the (scalar) error variance.  For a single
  group, the marginal covariance matrix of endog given exog is scale2*I
...
* The "natural parameterization" in which cov(endog) = scale2*I + Z *
...
* The "profile parameterization" in which cov(endog) = I +
  ...
  equal to the "profile" cov_re1 times scale2.
...
class MixedLM(base.LikelihoodModel):
        ...
        for itr in range(maxit):
            ...
            for j in range(self.k_fe):
                ...
                for k, lab in enumerate(self.group_labels):
                    ...
                    x = exog[:,j]
                    u = _smw_solve(scale2, ex_r, cov_re, cov_re_inv, x)
                    a += np.dot(u, x)
                    b -= 2*np.dot(u, resid)
    ...
    def loglike_full(self, params):
        ...
        the scale parameter scale2 has been profiled out.
        ...
        error variance scale2, and divide Psi by scale2.
        """
    ...
    def loglike(self, params):
        ...
        which the scale parameter scale2 has been profiled out.
    ...
    def score_full(self, params):
        ...
        parameter scale2 has been profiled out.
    ...
    def hessian_full(self, params):
        ...
        likelihood in which the scale parameter scale2 has been profiled
    ...
    def Estep(self, fe_params, cov_re, scale2):
        """
        ...
        for k in range(self.n_groups):
            ...
            vr1 = _smw_solve(scale2, ex_r, cov_re, cov_re_inv, resid)
            vr1 = np.dot(ex_r.T, vr1)
            vr1 = np.dot(cov_re, vr1)

            vr2 = _smw_solve(scale2, ex_r, cov_re, cov_re_inv,
                            self.exog_re_li[k])
    ...
    def EM(self, fe_params, cov_re, scale2, niter_em=10,
           hist=None):
        """
        ...
        Notes
        -----
        This uses the parameterization of the likelihood scale2*I +
        ...
        for itr in range(niter_em):
            ...
            m1x, m1y, m2, m2xx = self.Estep(fe_params, cov_re, scale2)
            ...
            scale2 = 0.
            for x,y in zip(self.exog_li, self.endog_li):
                scale2 += np.sum((y - np.dot(x, fe_params))**2)
            scale2 -= 2*m1y
            scale2 += 2*np.dot(fe_params, m1x)
            scale2 += np.trace(m2xx)
            scale2 /= self.n_totobs

            if hist is not None:
                hist.append(["EM", fe_params, cov_re, scale2])

        return fe_params, cov_re, scale2


    def get_scale2(self, fe_params, cov_re):
        """
    ...
    def fit(self, start=None, reml=True, niter_sd=1,
        ...
        start: dict
            ...
            `start["cov_re"]` is provided, then `start["scale2"]` must
            ...
            profile parameterization (in this case scale2 is not used).
        reml : bool
        ...
        if "cov_re_sqrt_unscaled" in start:
            ...
            cov_re_unscaled = start["cov_re"] / start["scale2"]
            if self.use_sqrt:
        ...
        if niter_em > 0:
            ...
            fe_params, cov_re, scale2 = self.EM(fe_params, cov_re, scale2,
                                             niter_em, hist)

            # Gradient algorithms use a different parameterization
            # that profiles out sigma^2.
            if self.use_sqrt:
                params_prof = self._pack(fe_params, cov_re / scale2)
            else:
                cov_re_rt = np.linalg.cholesky(cov_re / scale2)
        ...
        scale2 = self.get_scale2(fe_params, cov_re_unscaled)
        cov_re = scale2 * cov_re_unscaled
...
class MixedLMResults(base.LikelihoodModelResults):
    ...
    **Attributes**
    ...
    def bse_re(self):
        ...
        """
        p = self.model.exog.shape[1]
        return np.sqrt(self.scale2 * np.diag(self.cov_params())[p:])
    ...
    def ranef(self):
        ...
        for k in range(self.model.n_groups):
            ...
            vresid = _smw_solve(self.scale2, ex_r, self.cov_re,
                                cov_re_inv, resid)
    ...
    def ranef_cov(self):
        ...
        for k in range(self.model.n_groups):
            ...
            mat2 = _smw_solve(self.scale2, ex_r, self.cov_re, cov_re_inv,
                             mat1)
    ...
    def summary(self, yname=None, xname_fe=None, xname_re=None,
        ...
        for i in range(self.k_re):
            for j in range(i + 1):
                ...
                sdf[jj, 1] = np.sqrt(self.scale**2) * self.bse[jj]
                jj += 1
