    ...
    def density(self, xs, x):
        ...
        if len(xs) > 0:  # Need to do product of marginal distributions
            ...
            if self.weights is not None:
                w = np.mean(self((xs - x) * self._Hrootinv).T * self.weights) / sum(
                    self.weights
                )
            else:
...
class CustomKernel:
    ...
    def smooth(self, xs, ys, x):
        ...
        if len(xs) > 0:
            ...
            v = np.sum([yy * self((xx - x) / self.h) for xx, yy in zip(xs, ys, strict=False)])
            return v / w
        else:
            return np.nan
    ...
    def smoothvar(self, xs, ys, x):
        ...
        if len(xs) > 0:
            ...
            w = np.sum(self((xs - x) / self.h))
            v = np.sum([rr * self((xx - x) / self.h) for xx, rr in zip(xs, sqresid, strict=False)])
            return v / w
        else:
            return np.nan
    ...
    def L2Norm(self):
        ...
        if self._L2Norm is None:
            ...
            if self.domain is None:
                ...
                self._L2Norm = scipy.integrate.quad(
                    L2Func, self.domain[0], self.domain[1]
                )[0]
        return self._L2Norm
    ...
    def norm_const(self):
        ...
        if self._normconst is None:
            if self.domain is None:
                ...
                quadres = scipy.integrate.quad(
                    self._shape, self.domain[0], self.domain[1]
                )
    ...
    def kernel_var(self):
        ...
        if self._kernel_var is None:
            ...
            if self.domain is None:
                ...
                self._kernel_var = scipy.integrate.quad(
                    func, self.domain[0], self.domain[1]
                )[0]
...
class Uniform(CustomKernel):
    def __init__(self, h=1.0):
        CustomKernel.__init__(
            self,
            shape=lambda x: 0.5 * np.ones(x.shape),
            h=h,
            domain=[-1.0, 1.0],
            norm=1.0,
        )
...
class Triangular(CustomKernel):
    def __init__(self, h=1.0):
        CustomKernel.__init__(
            self, shape=lambda x: 1 - abs(x), h=h, domain=[-1.0, 1.0], norm=1.0
        )
...
class Biweight(CustomKernel):
    ...
    def smooth(self, xs, ys, x):
        ...
        if len(xs) > 0:
            ...
            v = np.sum(
                multiply(
                    ys, square(subtract(1, square(divide(subtract(xs, x), self.h))))
                )
            )
            return v / w
        else:
    ...
    def smoothvar(self, xs, ys, x):
        ...
        if len(xs) > 0:
            ...
            v = np.sum(
                multiply(
                    rs, square(subtract(1, square(divide(subtract(xs, x), self.h))))
                )
            )
            return v / w
        else:
...
class Triweight(CustomKernel):
    def __init__(self, h=1.0):
        CustomKernel.__init__(
            self,
            shape=lambda x: 1.09375 * (1 - x * x) ** 3,
            h=h,
            domain=[-1.0, 1.0],
            norm=1.0,
        )
...
class Gaussian(CustomKernel):
    ...
    def smooth(self, xs, ys, x):
        ...
        v = np.sum(
            multiply(ys, exp(multiply(square(divide(subtract(xs, x), self.h)), -0.5)))
        )
        return v / w
...
class Cosine2(CustomKernel):
    ...
    def __init__(self, h=1.0):
        CustomKernel.__init__(
            self,
            shape=lambda x: 1 + np.cos(2.0 * np.pi * x),
            h=h,
            domain=[-0.5, 0.5],
            norm=1.0,
        )
