...
class TukeyHSDResults:
    ...
    def _plot_error_bars(self,indices, color, ecolor, means, ax1, orientation='h'):
        ...
        if orientation == 'h':
            ax1.errorbar(data_points, indices, xerr=err, marker='o', linestyle='None', color=color, ecolor=ecolor)
        else:
            ax1.errorbar(indices, data_points, yerr=err, marker='o', linestyle='None', color=color, ecolor=ecolor)
    ...
    def plot_simultaneous(self, comparison_name=None, ax=None, figsize=(10,6),
                          xlabel=None, ylabel=None, vertical=False):
        ...
        vertical : bool, optional
            If True, the error bars will be vertical with groups on the x-axis and 
            the mean differences on the y-axis. Default is False, which plots horizontal bars.
        ...
        if comparison_name:
            sigidx, nsigidx, midx = self._process_comparison(comparison_name, minrange, maxrange)

        if vertical:
            if comparison_name:
                self._plot_error_bars([midx], 'b', 'b', means, ax1, orientation='v')
                ax1.plot([-1, self._multicomp.ngroups], [minrange[midx]]*2, linestyle='--', color='0.7')
                ax1.plot([-1, self._multicomp.ngroups], [maxrange[midx]]*2, linestyle='--', color='0.7')
                self._plot_error_bars(sigidx, 'r', 'r', means, ax1, orientation='v')
                self._plot_error_bars(nsigidx, '0.5', '0.5', means, ax1, orientation='v')
            else:
                self._plot_error_bars(range(len(means)), 'k', 'k', means, ax1, orientation='v')
        ...
        if vertical:
            ax1.set_ylim([np.min(minrange) - r / 10., np.max(maxrange) + r / 10.])
        else:
