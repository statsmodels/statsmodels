...
class HistDist:
    ...
    def optimize_binning(self, method="Freedman"):
        ...
        nobs = len(self.data)
        if method == "Freedman":
            IQR = self.ppf_emp(0.75) - self.ppf_emp(
                0.25
            )  # Interquantile range(75% -25%)
...
if __name__ == "__main__":
    ...
    if 1 in examples:
        ...
        emp = interpolate.InterpolatedUnivariateSpline(
            np.sort(x), np.sort(empiricalcdf(x)), k=1
        )
    ...
    ppfemp = interpolate.UnivariateSpline(
        np.sort(empiricalcdf(x)), np.sort(x), k=3, s=0.03
    )
