...
class Results:
    ...
    def predict(self, exog):
        ...
        if exog_smoothed.shape[0] == self.k_vars:
            ...
            warnings.warn(
                "old orientation, colvars, will go away", FutureWarning, stacklevel=2
            )
...
class AdditiveModel:
    ...
    def __init__(self, exog, smoothers=None, weights=None, family=None):
        ...
        self.smoothers = smoothers or [
            default_smoother(exog[:, i]) for i in range(exog.shape[1])
        ]
    ...
    def df_resid(self):
        ...
        return (
            ...
            - np.array(
                [self.smoothers[i].df_fit() for i in range(self.exog.shape[1])]
            ).sum()
        )
...
class Model(GLM, AdditiveModel):
    ...
    #        self.family = family
    def __init__(self, endog, exog, smoothers=None, family=GAUSSIAN_FAMILY):
        # self.family = family
