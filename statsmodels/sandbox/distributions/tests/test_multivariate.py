...
class Test_MVN_MVT_prob(object):
    ...
    def test_mvn_mvt_1(self):
        ...
        assert_allclose(probmvt_R, mvstdtprob(a, b, corr_equal, df), rtol=1e-4)
        assert_allclose(probmvn_R,
                        mvstdnormcdf(a, b, corr_equal, abseps=1e-5),
                        rtol=1e-4)

        mvn_high = mvstdnormcdf(a, b, corr_equal, abseps=1e-8, maxpts=10000000)
        assert_allclose(probmvn_R, mvn_high, rtol=1e-5)
        ...
        #>>> assert_allclose(0.67396999999999996, 0.67397072627419408, rtol=1e-6)
        #Fail
    ...
    def test_mvn_mvt_2(self):
        ...
        assert_allclose(probmvt_R, mvstdtprob(a, b, corr2, df), rtol=1e-4)
        assert_allclose(probmvn_R, mvstdnormcdf(a, b, corr2, abseps=1e-5), rtol=1e-4)
    ...
    def test_mvn_mvt_3(self):
        ...
        assert_allclose(probmvt_R, mvstdtprob(a2, b, corr2, df), rtol=1e-4)
        assert_allclose(probmvn_R,
                        mvstdnormcdf(a2, b, corr2, maxpts=100000, abseps=1e-5),
                        rtol=1e-4)
    ...
    def test_mvn_mvt_4(self):
        ...
        assert_allclose(probmvt_R,
                        mvstdtprob(np.zeros(3), -a2, corr2, df),
                        rtol=1e-4)
        assert_allclose(probmvn_R,
                        mvstdnormcdf(np.zeros(3), -a2, corr2,
                                     maxpts=100000, abseps=1e-5),
                        rtol=1e-4)
    ...
    def test_mvn_mvt_5(self):
        ...
        assert_allclose(mvstdtprob(a3, a3+1, corr2, df), probmvt_R, rtol=1e-4)
        assert_allclose(probmvn_R,
                        mvstdnormcdf(a3, a3+1, corr2,
                                     maxpts=100000, abseps=1e-5),
                        rtol=1e-4)
...
class TestMVDistributions(object):
    ...
    def test_mvn_pdf(self):
        ...
        assert_allclose(mvn3.logpdf(cov3), r_val, rtol=1e-14)
        #decimal 18
        r_val = [0.000467562492721686, 0.000989829804859273, 0.005696077243833402]
        assert_allclose(mvn3.pdf(cov3), r_val, rtol=1e-17)
        ...
        assert_allclose(mvn3b.pdf(cov3), r_val, rtol=1e-16)
    ...
    def test_mvt_pdf(self):
        ...
        mvt = MVT((0,0), 1, 5)
        assert_allclose(mvt.logpdf(np.array([0.,0.])), -1.837877066409345,
                        rtol=1e-15)
        assert_allclose(mvt.pdf(np.array([0.,0.])), 0.1591549430918953,
                        rtol=1e-15)
        ...
        rvs = mvt.rvs(100000)
        assert_allclose(np.cov(rvs, rowvar=0), mvt.cov, rtol=1e-1)

        mvt31 = MVT(mu3, cov3, 1)
        assert_allclose(mvt31.pdf(cov3),
            [0.0007276818698165781, 0.0009980625182293658, 0.0027661422056214652],
            rtol=1e-17)

        mvt = MVT(mu3, cov3, 3)
        assert_allclose(mvt.pdf(cov3),
