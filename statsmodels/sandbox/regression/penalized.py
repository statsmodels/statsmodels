...
class TheilGLS(GLS):
    ...
    def __init__(
        self, endog, exog, r_matrix=None, q_matrix=None, sigma_prior=None, sigma=None
    ):
    ...
    def fit(self, pen_weight=1.0, cov_type="sandwich", use_t=True):
        ...
        xx = np.dot(x.T, x)
        xpx = xx + sigma2_e * lambd * np.dot(
            r_matrix.T, np.dot(sigma_prior_inv, r_matrix)
        )
...
class TheilRegressionResults(RegressionResults):
    ...
    def test_compatibility(self):
        ...
        statistic = r_diff.T.dot(
            np.linalg.solve(ols_cov_r + self.model.sigma_prior, r_diff)
        )
