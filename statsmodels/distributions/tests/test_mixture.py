...
class TestMixtureDistributions:
    ...
    def test_mv_mixture_rvs_random(self):
        cov3 = np.array([[1.00, 0.5, 0.75], [0.50, 1.5, 0.60], [0.75, 0.6, 2.00]])
        mu = np.array([-1, 0.0, 2.0])
        mu2 = np.array([4, 2.0, 2.0])
        mvn3 = mvd.MVNormal(mu, cov3)
        mvn32 = mvd.MVNormal(mu2, cov3 / 2.0)
        rs = np.random.RandomState(0)
        res = mv_mixture_rvs([0.4, 0.6], 5000, [mvn3, mvn32], 3, rng=rs)
        npt.assert_almost_equal(
            np.array([res.std(), res.mean(), res.var()]),
    ...
    def test_mv_mixture_rvs_fixed(self):
        ...
        res = mv_mixture_rvs([0.2, 0.8], 10, [mvn3, mvn32], 3, rng=rs)
        npt.assert_almost_equal(
            res,
            np.array(
                [
