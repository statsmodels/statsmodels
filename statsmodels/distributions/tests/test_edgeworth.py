...
class TestFaaDiBruno:
    def test_neg_arg(self):
        with pytest.raises(ValueError):
            _faa_di_bruno_partitions(-1)
        with pytest.raises(ValueError):
            _faa_di_bruno_partitions(0)
...
class TestCumulants:
    def test_badvalues(self):
        with pytest.raises(ValueError):
            cumulant_from_moments([1, 2, 3], 0)
        with pytest.raises(ValueError):
            cumulant_from_moments([1, 2, 3], 4)
...
class TestExpandedNormal:
    ...
    def test_coefficients(self):
        with warnings.catch_warnings():
            ...
            assert_allclose(
                ne4._coef, [1.0, 0.0, 0.0, 1.0 / 6, 1.0 / 24, 0.0, 1.0 / 72]
            )
    ...
    def test_chi2_moments(self):
        ...
        # compare the moments
        assert_allclose(
            [_chi2_moment(n, df) for n in range(N)], [ne.moment(n) for n in range(N)]
        )
...
def check_pdf(distfn, arg, msg):
    ...
    npt.assert_almost_equal(
        pdfv, cdfdiff, decimal=DECIMAL, err_msg=msg + " - cdf-pdf relationship"
    )
