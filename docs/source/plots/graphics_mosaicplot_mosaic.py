...
'''
A more useful example is given by a dictionary with multiple indices.  In this
case we use a wider gap to a better visual separation of the resulting plot
'''
...
'''
The third accepted data structureis the np array, for which a very simple index
will be created.
'''
rand = np.random.random
data = 1+rand((2, 2))
...
'''
If you need to modify the labeling and the coloring you can give a function to
