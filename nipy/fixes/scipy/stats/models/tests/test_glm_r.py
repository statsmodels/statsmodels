...
def assert_model_similar_base(res1, res2):
    ...
    JP: where are the params standard errors or params covariance - now included
    res1 is from R'''
    nptest.assert_almost_equal(res1.params, res2.params, 4)
...
def assert_model_similar_glm(res1, res2):
    ...
    this doesn't work'''
    yield nptest.assert_almost_equal, res1.params, res2.results.params, 4
...
class Check_Glm_R(object):
    def test_params(self):
        nptest.assert_array_almost_equal(self.res1.params,
                                self.res2.results.params, DECIMAL)
    def test_bse(self):
        nptest.assert_array_almost_equal(self.res1.bse,
                                self.res2.results.bse, DECIMAL)
#    def test_bcov(self):
#        nptest.assert_array_almost_equal(self.res1.bcov,
#                                self.res2.results.cov_params(), DECIMAL)
#   need to sort out cov_params and normalized_cov_params

    def test_deviance(self):
        nptest.assert_array_almost_equal(self.res1.deviance,
                                self.res2.results.deviance, DECIMAL)
    def test_fitted(self):
        nptest.assert_array_almost_equal(self.res1.predictedy,
                                self.res2.results.mu, DECIMAL)
    def test_fitted(self):
        nptest.assert_array_almost_equal(self.res1.resid,
                                self.res2.resid, DECIMAL)
    def test_fitted(self):
        nptest.assert_array_almost_equal(self.res1.predict,
                                self.res2.results.predict, DECIMAL)
    def test_scale(self):
        nptest.assert_array_almost_equal(self.res1.scale,
                                self.res2.results.scale, DECIMAL)
    @nptest.dec.knownfailureif(True, "AIC is defined and calculated\
 differently than R.")
    def test_aic(self):
    ...
    def test_df(self):
        ...
        nptest.assert_almost_equal(self.res1.df[2], self.res2.df_model+1, 4)
...
#        glmols_res.params = glmols_res.params
...
class test_glm_binomial(Check_Glm_R):
    def __init__(self):
        ...
        des = np.column_stack((X['age'],X['lwt'],X['black'],X['other'],
                    X['smoke'], X['ptl'], X['ht'], X['ui']))
...
##        print glmbin_res.params
##        print rglmbin_res.params
...
class _est_glm_binomial_fraction(Check_Glm_R):
    def __init__(self):
        ...
        model_glmbinfrac = GlmModel(data.endog, data.exog,
...
##        print glmgam_res.params
##        print rglmgam_res.params
...
class test_glm_poisson(Check_Glm_R):
    def __init__(self):
        ...
        print rglmtestpoiss_res.params
...
##        print glmtestgaulog_res.params
##        print rglmtestgaulog_res.params
...
##        print glmtestgauinv_res.params
##        print rglmtestgauinv_res.params
