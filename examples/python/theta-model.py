...
# 2. Deseasonalize if seasonality is detected
# 3. Estimate $\alpha$ by fitting an SES model to the data and $b_0$ by
# OLS.
...
# lag is significantly different from zero, then the data is deseasonalized
# using `statsmodels.tsa.seasonal_decompose` using either a multiplicative
...
# We specify the model without any options and fit it. The summary shows
# that the data was deseasonalized using the multiplicative method.  The
# drift is modest and negative, and the smoothing parameter is fairly low.
...
# produced using the `forecast` method from the fitted model. Below, we
# produce a hedgehog plot by forecasting 2-years ahead every 2 years.
#
...
# Next we explore differences in the forecast as $\theta$ changes. When
...
# We conclude by producing a hedgehog plot using 2-year non-overlapping
# samples.
