...
legend.prop.set_size(20)
_ = ax.set_title("Real GDP and the extradted trend using the HP filter")
...
_ = ax.set_title("Cycle extracted using Baxter & King's filter")
...
_ = ax.set_title("Cycle extracted using Christiano & Fitzgerald's filter")
...
# ### Hamilton Filter
#
# Hamilton (2018) proposes a new method for estimating the cyclical
# component of a time series. The Hamilton filter is based on the idea that
# the cyclical component can be estimated by regressing the series on its
# own lags and then taking the residuals as the cyclical component. This
# method does not require any assumptions about the frequency of the cycle
# or the smoothness of the trend, making it more flexible than traditional
# filters. The filter takes two inputs, `p`, the number of lags to include
# in the model and `h`, the number of leads to use for the forecast. The
# cyclical component is then estimated as the residuals from the regression
# of the series on its own lags and leads. The underling AR model estimated
# is
#
# $$
...
# $$
#
# The recommendations are ``h=8, p=4`` for quarterly data and ``h=24,
# p=12`` for monthly data.
#
# The Hamilton filter fills the first $p+h-1$ places with `NaN` since
# there is no data available to estimate the AR in these positions.


h_cycles, h_trend = sm.tsa.filters.hamilton_filter(dta[["infl", "unemp"]], h=8, p=4)
print(h_cycles.head(13))


fig = plt.figure(figsize=(14, 10))
ax = fig.add_subplot(111)
h_cycles.plot(ax=ax, style=["r--", "b-"])
_ = ax.set_title("Cycle extracted using Hamilton's filter")
