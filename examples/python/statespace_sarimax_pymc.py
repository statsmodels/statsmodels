...
# DO NOT EDIT
# Autogenerated from the notebook statespace_sarimax_pymc.ipynb.
...
# econometrics, and [PyMC](https://www.pymc.io/), which is for Bayesian
# estimation, to perform fast Bayesian estimation of a simple SARIMAX model,
# in this case an ARMA(1, 1) model for US CPI.
#
# Note that, for simple models like AR(p), base PyMC is a quicker way to
# fit a model; there are examples in the [PyMC example
# gallery](https://www.pymc.io/projects/examples/en/latest/gallery.html).
# The advantage of using statsmodels is that it gives access to methods that
# can solve a vast range of statespace models.
#
...
# ### Technical Details
#
# PyMC is a Bayesian estimation library ("Bayesian Modeling and
# Probabilistic Programming in Python") that is a) fast and b) optimized for
# Bayesian machine learning, for instance Bayesian neural networks. To do
# all of this, it is built on top of PyTensor, a library that aims to
# evaluate tensors very efficiently and provide symbolic differentiation
# (necessary for any kind of deep learning). It is the symbolic
# differentiation that means PyMC can use NUTS on any problem formulated
# within PyMC.
#
# We are not formulating a problem directly in PyMC; we're using
# statsmodels to specify the statespace model and solve it with the Kalman
# filter. So we need to put the plumbing of statsmodels and PyMC together,
# which means wrapping the statsmodels SARIMAX model object in a PyTensor-
# flavored wrapper before passing information to PyMC for estimation.
#
# Because of this, we can't use PyTensor's auto-differentiation directly.
...
# #### Defining helper functions to translate models into a PyMC friendly
# form
#
# First, we'll create the PyTensor wrappers. They will be in the form of
...
# gradients. PyMC will use Metropolis or Slicing samplers if it does not
# find that gradients are available. There are an impressive number of draws
# per second for a "black box" style computation! However, note that if the
# model can be represented directly by PyMC (like the AR(p) models mentioned
# above), then computation can be substantially faster.
#
...
# As noted earlier, the PyTensor wrappers (`Loglike` and `Score`) that we
