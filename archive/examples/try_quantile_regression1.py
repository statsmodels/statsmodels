...
mix = 0.1 * stats.norm.pdf(x[:, None], loc=np.linspace(-0.5, 0.75, 4), scale=0.01).sum(
    1
)
...
for lab, beta in zip(["ols", "qr 0.1", "qr 0.5", "qr 0.75"], params, strict=True):
    print(f"{lab!s:<8}", np.round(beta, 4))
