...
class MyPareto(GenericLikelihoodModel):
    ...
    def nloglikeobs(self, params):
        ...
        endog = self.endog
        x = (endog - loc) / scale
        logpdf = np_log(b) - (b + 1.0) * np_log(x)  # use np_log(1 + x) for Pareto II
        logpdf -= np.log(scale)
        # lb = loc + scale
        # logpdf[endog<lb] = -inf
...
res_par = mod_par.fit(
    start_params=mod_par.start_params, method="nm", maxfun=10000, maxiter=5000
)
...
mod_par.data.xnames = [
    name
    for (name, incl) in zip(["shape", "loc", "scale"], mod_par.fixed_paramsmask, strict=True)
    if incl
]
