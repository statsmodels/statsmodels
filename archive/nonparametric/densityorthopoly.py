...
class HPoly:
    ...
    def __call__(self, x):
        ...
        lnfact = (
            -(1.0 / 2) * (k * np.log(2.0) + special.gammaln(k + 1) + logpi2) - x * x / 2
        )
        fact = np.exp(lnfact)
...
class DensityOrthoPoly:
    ...
    def evaluate(self, xeval, order=None):
        ...
        res = sum(c * p(xeval) for c, p in list(zip(self.coeffs, self.polys, strict=True))[:order])
...
def density_orthopoly(x, polybase, order=5, xeval=None):
    ...
    coeffs = [(p(x)).mean() for p in polys]
    res = sum(c * p(xeval) for c, p in zip(coeffs, polys, strict=True))
    # res *= (1-xeval**2)**(-1/2.)
...
if __name__ == "__main__":
    ...
    if "chebyt_" in examples:  # needed for Cheby example below
        ...
        f_hat, grid, coeffs, polys = density_orthopoly(
            obs_dist, ChebyTPoly, order=20, xeval=None
        )
