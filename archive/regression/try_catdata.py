...
def labelmeanfilter_nd(y, x):
    ...
    labelcount = np.array(
        ndimage.histogram(
            y, labelsunique[0], labelsunique[-1] + 1, 1, labels=y, index=labelsunique
        )
    )
...
def labelmeanfilter_str(ys, x):
    ...
    labelmeans = np.array(
        ndimage.mean(x, labels=unilinv, index=np.arange(np.max(unil) + 1))
    )
...
def convertlabels(ys, indices=None):
    ...
    if indices is None:
        ...
        if idx.size > 1 and ys.ndim == 2:
            ylabel = np.array([f"@{ii[:2].tostring()}@" for ii in ys])[:, np.newaxis]
            # alternative
