...
x = sm.tsa.tsatools.lagmat2ds(np.column_stack((y22, e)), 3, dropex=1, trim="both")
yt = x[:, 0]
xt = x[:, 1:]
...
start_params_mle[:-1] = np.sign(start_params_mle[:-1]) * np.minimum(
    np.abs(start_params_mle[:-1]), 0.75
)
