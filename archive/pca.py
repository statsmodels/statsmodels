...
class Pca:
    ...
    def plot2d(self, ix=0, iy=1, clf=True):
        ...
        yl, yu = plt.ylim()
        dx, dy = (xu - xl), (yu - yl)
        for val, vec, c in zip(vals, evs.T, self._colors, strict=True):
            plt.arrow(
                0,
                0,
                val * vec[ix],
                val * vec[iy],
                head_width=0.05 * (dx * dy / 4) ** 0.5,
                fc=c,
                ec=c,
            )
    ...
    def plot3d(self, ix=0, iy=1, iz=2, clf=True):
        ...
        if self.names:
            M.axes(
                xlabel=self.names[ix] + "/sigma",
                ylabel=self.names[iy] + "/sigma",
                zlabel=self.names[iz] + "/sigma",
            )
        else:
            M.axes()
